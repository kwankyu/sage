...
def NonisotropicUnitaryPolarGraph(m, q):
    ...
    TESTS::
        ...
        (540, 224, 88, 96)
        sage: graphs.NonisotropicUnitaryPolarGraph(6,6)                                 # needs sage.libs.gap
        Traceback (most recent call last):
        ...
...
def Nowhere0WordsTwoWeightCodeGraph(q, hyperoval=None, field=None, check_hyperoval=True):
    ...
    supplying your own hyperoval::
        ...
        sage: F = GF(8)
        sage: O = [vector(F,(0,0,1)),vector(F,(0,1,0))] + [vector(F, (1,x^2,x))         # needs sage.rings.finite_rings
        ....:                                              for x in F]
        sage: g = graphs.Nowhere0WordsTwoWeightCodeGraph(8,hyperoval=O,field=F); g      # needs sage.rings.finite_rings
        Nowhere0WordsTwoWeightCodeGraph(8): Graph on 196 vertices
        sage: g.is_strongly_regular(parameters=True)                                    # needs sage.rings.finite_rings
        (196, 60, 14, 20)
