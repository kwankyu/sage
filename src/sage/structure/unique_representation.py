...
different finite fields are all equal to the integer one, we find::
    ...
    sage: GF(5)(1) == 1 == GF(3)(1)                                                     # needs sage.rings.finite_rings
    True
    sage: C(1) is C(GF(3)(1)) is C(GF(5)(1))                                            # needs sage.rings.finite_rings
    True
...
instances of quite different classes::
    ...
    True
    sage: K5 = GF(5)                                                                                # needs sage.rings.finite_rings
    sage: type(K5)                                                                                  # needs sage.rings.finite_rings
...
class UniqueRepresentation(CachedRepresentation, WithEqualityById):
    ...
    This nice behaviour is not available when one just uses a factory::
        ...
        sage: isinstance(GF(7), GF)                                                     # needs sage.rings.finite_rings
        Traceback (most recent call last):
        ...
        TypeError: isinstance() arg 2 must be a type...

        sage: isinstance(GF, sage.structure.factory.UniqueFactory)                      # needs sage.rings.finite_rings
        True
