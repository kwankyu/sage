...
cdef class NamedConvertMap(Map):
    ...
    def __init__(self, domain, codomain, method_name):
        ...
        EXAMPLES::
            ...
            t
            sage: mor = NamedConvertMap(SR, QQ['t'], '_polynomial_')                    # needs sage.symbolic
            sage: mor(t^2/4 + 1)                                                        # needs sage.symbolic
            1/4*t^2 + 1
            sage: mor = NamedConvertMap(SR, GF(7)[['t']], '_polynomial_')               # needs sage.symbolic
            sage: mor(t^2/4 + 1)                                                        # needs sage.symbolic
            1 + 2*t^2
        """
    ...
    cdef dict _extra_slots(self):
        ...
        EXAMPLES::
            ...
            sage: var('t')                                                              # needs sage.symbolic
            t
            sage: phi = NamedConvertMap(SR, QQ['t'], '_polynomial_')                    # needs sage.symbolic
            sage: psi = copy(phi)    # indirect doctest                                 # needs sage.symbolic
            sage: psi                                                                   # needs sage.symbolic
            Conversion via _polynomial_ method map:
              From: Symbolic Ring
              To:   Univariate Polynomial Ring in t over Rational Field
            sage: phi == psi                    # not implemented                       # needs sage.symbolic
            True
            sage: psi(t^2/4 + 1)                                                        # needs sage.symbolic
            1/4*t^2 + 1
            sage: psi(t^2/4 + 1) == phi(t^2/4 + 1)                                      # needs sage.symbolic
            True
        """
    ...
    cpdef Element _call_(self, x):
        ...
        EXAMPLES::
            ...
            sage: from sage.structure.coerce_maps import NamedConvertMap
            sage: f = NamedConvertMap(GF(5), QQ, '_integer_'); f                        # needs sage.rings.finite_rings
            Conversion via _integer_ method map:
              From: Finite Field of size 5
              To:   Rational Field
            sage: f(19)                                                                 # needs sage.rings.finite_rings
            4
            sage: f(19).parent()                                                        # needs sage.rings.finite_rings
            Rational Field
        """
        cdef Parent C = self._codomain
        try:
