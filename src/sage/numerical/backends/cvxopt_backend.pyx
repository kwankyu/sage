...
cdef class CVXOPTBackend(GenericBackend):
    ...
    cpdef __copy__(self):
        ...
        EXAMPLES::
            ...
            sage: from sage.numerical.backends.generic_backend import get_solver
            sage: p = MixedIntegerLinearProgram(solver = "CVXOPT")                      # needs cvxopt
            sage: b = p.new_variable()                                                  # needs cvxopt
            sage: p.add_constraint(b[1] + b[2] <= 6)                                    # needs cvxopt
            sage: p.add_constraint(b[2] <= 5)                                           # needs cvxopt
            sage: p.set_objective(b[1] + b[2])                                          # needs cvxopt
            sage: cp = copy(p.get_backend())                                            # needs cvxopt
            sage: cp.solve()                                                            # needs cvxopt
            0
            sage: cp.get_objective_value()                                              # needs cvxopt
            6.0
        """
    ...
    cpdef int add_variable(self, lower_bound=0.0, upper_bound=None, binary=False, continuous=True, integer=False, obj=None, name=None) except -1:
        ...
        EXAMPLES::
            ...
            sage: from sage.numerical.backends.generic_backend import get_solver
            sage: p = get_solver(solver = "CVXOPT")                                     # needs cvxopt
            sage: p.ncols()                                                             # needs cvxopt
            0
            sage: p.add_variable()                                                      # needs cvxopt
            0
            sage: p.ncols()                                                             # needs cvxopt
            1
            sage: p.add_variable()                                                      # needs cvxopt
            1
            sage: p.add_variable(lower_bound=-2.0)                                      # needs cvxopt
            2
            sage: p.add_variable(continuous=True)                                       # needs cvxopt
            3
            sage: p.add_variable(name='x',obj=1.0)                                      # needs cvxopt
            4
            sage: p.col_name(3)                                                         # needs cvxopt
            'x_3'
            sage: p.col_name(4)                                                         # needs cvxopt
            'x'
            sage: p.objective_coefficient(4)                                            # needs cvxopt
            1.00000000000000
    ...
    cpdef set_variable_type(self, int variable, int vtype):
        ...
        EXAMPLES::
            ...
            sage: from sage.numerical.backends.generic_backend import get_solver
            sage: p = get_solver(solver = "cvxopt")                                     # needs cvxopt
            sage: p.add_variables(5)                                                    # needs cvxopt
            4
            sage: p.set_variable_type(3, -1)                                            # needs cvxopt
            sage: p.set_variable_type(3, -2)                                            # needs cvxopt
            Traceback (most recent call last):
            ...
            ValueError: ...
        """
    ...
    cpdef set_sense(self, int sense):
        ...
        EXAMPLES::
            ...
            sage: from sage.numerical.backends.generic_backend import get_solver
            sage: p = get_solver(solver = "CVXOPT")                                     # needs cvxopt
            sage: p.is_maximization()                                                   # needs cvxopt
            True
            sage: p.set_sense(-1)                                                       # needs cvxopt
            sage: p.is_maximization()                                                   # needs cvxopt
            False
        """
        if sense == 1:
    ...
    cpdef objective_coefficient(self, int variable, coeff=None):
        ...
        EXAMPLES::
            ...
            sage: p.add_variable()                                                      # needs cvxopt
            0
            sage: p.objective_coefficient(0)                                            # needs cvxopt
            0.0
            sage: p.objective_coefficient(0,2)                                          # needs cvxopt
            sage: p.objective_coefficient(0)                                            # needs cvxopt
            2.0
        """
    ...
    cpdef add_col(self, indices, coeffs):
        ...
        EXAMPLES::
            ...
            sage: p.ncols()                                                             # needs cvxopt
            0
            sage: p.nrows()                                                             # needs cvxopt
            0
            sage: p.add_linear_constraints(5, 0, None)                                  # needs cvxopt
            sage: p.add_col(range(5), range(5))                                         # needs cvxopt
            sage: p.nrows()                                                             # needs cvxopt
            5
        """
        column = []
    ...
    cpdef add_linear_constraint(self, coefficients, lower_bound, upper_bound, name=None):
        ...
        EXAMPLES::
            ...
            sage: p.add_variables(5)                                                    # needs cvxopt
            4
            sage: p.add_linear_constraint(zip(range(5), range(5)), 2.0, 2.0)            # needs cvxopt
            sage: p.row(0)                                                              # needs cvxopt
            ([1, 2, 3, 4], [1, 2, 3, 4])
            sage: p.row_bounds(0)                                                       # needs cvxopt
            (2.00000000000000, 2.00000000000000)
            sage: p.add_linear_constraint(zip(range(5), range(5)), 1.0, 1.0, name='foo')            # needs cvxopt
            sage: p.row_name(-1)                                                        # needs cvxopt
            'foo'
        """
    ...
    cpdef int solve(self) except -1:
        ...
        EXAMPLES::
            ...
            sage: p = MixedIntegerLinearProgram(solver = "cvxopt", maximization=False)  # needs cvxopt
            sage: x=p.new_variable(nonnegative=True)                                    # needs cvxopt
            sage: p.set_objective(-4*x[0] - 5*x[1])                                     # needs cvxopt
            sage: p.add_constraint(2*x[0] + x[1] <= 3)                                  # needs cvxopt
            sage: p.add_constraint(2*x[1] + x[0] <= 3)                                  # needs cvxopt
            sage: N(p.solve(), digits=2)                                                # needs cvxopt
            -9.0
            sage: p = MixedIntegerLinearProgram(solver = "cvxopt", maximization=False)  # needs cvxopt
            sage: x=p.new_variable(nonnegative=True)                                    # needs cvxopt
            sage: p.set_objective(x[0] + 2*x[1])                                        # needs cvxopt
            sage: p.add_constraint(-5*x[0] + x[1]  <=   7)                              # needs cvxopt
            sage: p.add_constraint(-5*x[0] + x[1]  >=   7)                              # needs cvxopt
            sage: p.add_constraint(x[0] + x[1] >= 26  )                                 # needs cvxopt
            sage: p.add_constraint( x[0] >= 3)                                          # needs cvxopt
            sage: p.add_constraint( x[1] >= 4)                                          # needs cvxopt
            sage: N(p.solve(),digits=4)                                                 # needs cvxopt
            48.83
            sage: p = MixedIntegerLinearProgram(solver = "cvxopt")                      # needs cvxopt
            sage: x=p.new_variable(nonnegative=True)                                    # needs cvxopt
            sage: p.set_objective(x[0] + x[1] + 3*x[2])                                 # needs cvxopt
            sage: p.solver_parameter("show_progress",True)                              # needs cvxopt
            sage: p.add_constraint(x[0] + 2*x[1] <= 4)                                  # needs cvxopt
            sage: p.add_constraint(5*x[2] - x[1] <= 8)                                  # needs cvxopt
            sage: N(p.solve(), digits=2)                                                # needs cvxopt
                     pcost       dcost       gap    pres   dres   k/t
                 ...
                8.8
            sage: #CVXOPT gives different  values for variables compared to the other solvers.
            sage: c = MixedIntegerLinearProgram(solver = "cvxopt")                      # needs cvxopt
            sage: p = MixedIntegerLinearProgram(solver = "ppl")                         # needs cvxopt
            sage: g = MixedIntegerLinearProgram()                                       # needs cvxopt
            sage: xc=c.new_variable(nonnegative=True)                                   # needs cvxopt
            sage: xp=p.new_variable(nonnegative=True)                                   # needs cvxopt
            sage: xg=g.new_variable(nonnegative=True)                                   # needs cvxopt
            sage: c.set_objective(xc[2])                                                # needs cvxopt
            sage: p.set_objective(xp[2])                                                # needs cvxopt
            sage: g.set_objective(xg[2])                                                # needs cvxopt
            sage: #we create a cube for all three solvers
            sage: c.add_constraint(xc[0] <= 100)                                        # needs cvxopt
            sage: c.add_constraint(xc[1] <= 100)                                        # needs cvxopt
            sage: c.add_constraint(xc[2] <= 100)                                        # needs cvxopt
            sage: p.add_constraint(xp[0] <= 100)                                        # needs cvxopt
            sage: p.add_constraint(xp[1] <= 100)                                        # needs cvxopt
            sage: p.add_constraint(xp[2] <= 100)                                        # needs cvxopt
            sage: g.add_constraint(xg[0] <= 100)                                        # needs cvxopt
            sage: g.add_constraint(xg[1] <= 100)                                        # needs cvxopt
            sage: g.add_constraint(xg[2] <= 100)                                        # needs cvxopt
            sage: N(c.solve(),digits=4)                                                 # needs cvxopt
            100.0
            sage: N(c.get_values(xc[0]),digits=3)                                       # needs cvxopt
            50.0
            sage: N(c.get_values(xc[1]),digits=3)                                       # needs cvxopt
            50.0
            sage: N(c.get_values(xc[2]),digits=4)                                       # needs cvxopt
            100.0
            sage: N(p.solve(),digits=4)                                                 # needs cvxopt
            100.0
            sage: N(p.get_values(xp[0]),2)                                              # needs cvxopt
            0.00
            sage: N(p.get_values(xp[1]),2)                                              # needs cvxopt
            0.00
            sage: N(p.get_values(xp[2]),digits=4)                                       # needs cvxopt
            100.0
            sage: N(g.solve(),digits=4)                                                 # needs cvxopt
            100.0
            sage: N(g.get_values(xg[0]),2)                                              # needs cvxopt
            0.00
            sage: N(g.get_values(xg[1]),2)                                              # needs cvxopt
            0.00
            sage: N(g.get_values(xg[2]),digits=4)                                       # needs cvxopt
            100.0
        """
    ...
    cpdef get_objective_value(self):
        ...
        EXAMPLES::
            ...
            sage: from sage.numerical.backends.generic_backend import get_solver
            sage: p = get_solver(solver = "cvxopt")                                     # needs cvxopt
            sage: p.add_variables(2)                                                    # needs cvxopt
            1
            sage: p.add_linear_constraint([(0,1), (1,2)], None, 3)                      # needs cvxopt
            sage: p.set_objective([2, 5])                                               # needs cvxopt
            sage: p.solve()                                                             # needs cvxopt
            0
            sage: N(p.get_objective_value(),4)                                          # needs cvxopt
            7.5
            sage: N(p.get_variable_value(0),4)                                          # needs cvxopt
            3.6e-7
            sage: N(p.get_variable_value(1),4)                                          # needs cvxopt
            1.5
        """
    ...
    cpdef int nrows(self):
        ...
        EXAMPLES::
            ...
            sage: p.add_variables(5)                                                    # needs cvxopt
            4
            sage: p.add_linear_constraints(2, 2.0, None)                                # needs cvxopt
            sage: p.nrows()                                                             # needs cvxopt
            2
        """
    ...
    cpdef problem_name(self, name=None):
        ...
        EXAMPLES::
            ...
            sage: from sage.numerical.backends.generic_backend import get_solver
            sage: p = get_solver(solver = "CVXOPT")                                     # needs cvxopt
            sage: p.problem_name()                                                      # needs cvxopt
            ''
            sage: p.problem_name("There once was a french fry")                         # needs cvxopt
            sage: print(p.problem_name())                                               # needs cvxopt
    ...
    cpdef row(self, int i):
        ...
        EXAMPLES::
            ...
            sage: p.add_variables(5)                                                    # needs cvxopt
            4
            sage: p.add_linear_constraint(list(zip(range(5), range(5))), 2, 2)          # needs cvxopt
            sage: p.row(0)                                                              # needs cvxopt
            ([1, 2, 3, 4], [1, 2, 3, 4])
            sage: p.row_bounds(0)                                                       # needs cvxopt
            (2, 2)
    ...
    cpdef col_bounds(self, int index):
        ...
        EXAMPLES::
            ...
            sage: p.add_variable()                                                      # needs cvxopt
            0
            sage: p.col_bounds(0)                                                       # needs cvxopt
            (0.0, None)
            sage: p.variable_upper_bound(0, 5)                                          # needs cvxopt
            sage: p.col_bounds(0)                                                       # needs cvxopt
            (0.0, 5)
        """
    ...
    cpdef bint is_variable_binary(self, int index):
        ...
        EXAMPLES::
            ...
            sage: p.add_variable()                                                      # needs cvxopt
            0
            sage: p.set_variable_type(0,0)                                              # needs cvxopt
            Traceback (most recent call last):
            ...
            ValueError: ...
            sage: p.is_variable_binary(0)                                               # needs cvxopt
            False
    ...
    cpdef bint is_variable_integer(self, int index):
        ...
        EXAMPLES::
            ...
            sage: p.add_variable()                                                      # needs cvxopt
            0
            sage: p.set_variable_type(0,-1)                                             # needs cvxopt
            sage: p.set_variable_type(0,1)                                              # needs cvxopt
            Traceback (most recent call last):
            ...
            ValueError: ...
            sage: p.is_variable_integer(0)                                              # needs cvxopt
            False
        """
        return False
    ...
    cpdef bint is_variable_continuous(self, int index):
        ...
        EXAMPLES::
            ...
            sage: p.add_variable()                                                      # needs cvxopt
            0
            sage: p.is_variable_continuous(0)                                           # needs cvxopt
            True
            sage: p.set_variable_type(0,1)                                              # needs cvxopt
            Traceback (most recent call last):
            ...
    ...
    cpdef variable_lower_bound(self, int index, value = False):
        ...
        EXAMPLES::
            ...
            sage: p.col_bounds(0)                                                       # needs cvxopt
            (0.0, None)
            sage: p.variable_lower_bound(0, 5)                                          # needs cvxopt
            sage: p.col_bounds(0)                                                       # needs cvxopt
            (5, None)
        """
        if value is not False:
    ...
    cpdef solver_parameter(self, name, value = None):
        ...
        EXAMPLES::
            ...
            sage: from sage.numerical.backends.generic_backend import get_solver
            sage: p = get_solver(solver="CVXOPT")                                       # needs cvxopt
            sage: p.solver_parameter("show_progress")                                   # needs cvxopt
            False
            sage: p.solver_parameter("show_progress", True)                             # needs cvxopt
            sage: p.solver_parameter("show_progress")                                   # needs cvxopt
            True
