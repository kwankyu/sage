...
Introspection works::
    ...
    sage: print(sage_getdoc(test_pfunc))                                                # needs sage.misc.cython
       Some documentation
    sage: print(sage_getdoc(O.wrapped_method))                                          # needs sage.misc.cython
    some doc for a wrapped cython method
    <BLANKLINE>
    sage: print(sage_getdoc(O.direct_method))                                           # needs sage.misc.cython
    Some doc for direct method
    <BLANKLINE>
    sage: print(sage_getsource(O.wrapped_method))                                       # needs sage.misc.cython
    cpdef test_meth(self,x):
        "some doc for a wrapped cython method"
        return -x
    sage: print(sage_getsource(O.direct_method))                                        # needs sage.misc.cython
    def direct_method(self, x):
        "Some doc for direct method"
        return 2*x
...
``_cache_key``::
    ...
    sage: @cached_method                                                                # needs sage.misc.cython
    ....: def f(x): return x == a
