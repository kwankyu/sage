...
cdef class IntegerMod_abstract(FiniteRingElement):
    ...
    def _sage_input_(self, sib, coerced):
        ...
        EXAMPLES::
            ...
            sage: K = GF(7)                                                             # needs sage.rings.finite_rings
            sage: sage_input(K(5), verify=True)                                         # needs sage.rings.finite_rings
            # Verified
            GF(7)(5)
            sage: sage_input(K(5) * polygen(K), verify=True)                            # needs sage.rings.finite_rings
            # Verified
            R.<x> = GF(7)[]
            5*x
            sage: from sage.misc.sage_input import SageInputBuilder
            sage: K(5)._sage_input_(SageInputBuilder(), False)                          # needs sage.rings.finite_rings
            {call: {call: {atomic:GF}({atomic:7})}({atomic:5})}
            sage: K(5)._sage_input_(SageInputBuilder(), True)                           # needs sage.rings.finite_rings
            {atomic:5}
        """
        v = sib.int(self.lift())
        if coerced:
    ...
    def log(self, b=None, logarithm_exists=None):
        ...
        or the inputs are not units::
            ...
            sage: a = Mod(16, 100); b = Mod(4, 100)                                     # needs sage.libs.pari
            sage: a.log(b)                                                              # needs sage.libs.pari
            Traceback (most recent call last):
            ...
        ...
        We test that :trac:`12419` is fixed::
            ...
            sage: R(1).factor()                                                         # needs sage.rings.finite_rings
            1
...
#        assert b**n == self
    ...
    def minpoly(self, var='x'):
        ...
        EXAMPLES::
            ...
            sage: GF(241, 'a')(1).minpoly()                                             # needs sage.rings.finite_rings
            x + 240
        """
    ...
    def minimal_polynomial(self, var='x'):
        ...
        EXAMPLES::
            ...
            sage: GF(241, 'a')(1).minimal_polynomial(var = 'z')                         # needs sage.rings.finite_rings
            z + 240
        """
    ...
    def norm(self):
        ...
        EXAMPLES::
            ...
            sage: k = GF(691)                                                           # needs sage.rings.finite_rings
            sage: a = k(389)                                                            # needs sage.rings.finite_rings
            sage: a.norm()                                                              # needs sage.rings.finite_rings
            389
    ...
    def trace(self):
        ...
        EXAMPLES::
            ...
            sage: k = GF(691)                                                           # needs sage.rings.finite_rings
            sage: a = k(389)                                                            # needs sage.rings.finite_rings
            sage: a.trace()                                                             # needs sage.rings.finite_rings
            389
    ...
    def sqrt(self, extend=True, all=False):
        ...
        EXAMPLES::
            ...
            14
            sage: mod(15, 389).sqrt(extend=False)                                       # needs sage.libs.pari
            Traceback (most recent call last):
            ...
    ...
    def nth_root(self, n, extend = False, all = False, algorithm = None, cunningham = False):
        ...
        EXAMPLES::
            ...
            sage: K = GF(31)                                                            # needs sage.rings.finite_rings
            sage: a = K(22)                                                             # needs sage.rings.finite_rings
            sage: K(22).nth_root(7)                                                     # needs sage.rings.finite_rings
            13
            sage: K(25).nth_root(5)                                                     # needs sage.rings.finite_rings
            5
            sage: K(23).nth_root(3)                                                     # needs sage.rings.finite_rings
            29
            ...
            [1, 7, 5, 3]
            sage: mod(4,8).nth_root(2, all=True)                                        # needs sage.rings.padics
            [2, 6]
            sage: mod(1,16).nth_root(4, all=True)                                       # needs sage.rings.padics
            [1, 15, 13, 3, 9, 7, 5, 11]
            sage: (mod(22,31)^200).nth_root(200)                                        # needs sage.groups
            5
            sage: mod(3,6).nth_root(0, all=True)                                        # needs sage.rings.padics
            []
            sage: mod(3,6).nth_root(0)
            Traceback (most recent call last):
            ...
            ValueError
            sage: mod(1,6).nth_root(0, all=True)                                        # needs sage.rings.padics
            [1, 2, 3, 4, 5]
    ...
    def is_primitive_root(self):
        ...
        EXAMPLES::
            ...
            True
            sage: mod(2, 7).is_primitive_root()                                         # needs sage.libs.pari
            False
        ...
        `0` is not a primitive root mod `n` (:trac:`23624`) except for `n=0`::
            ...
            sage: mod(0, 17).is_primitive_root()                                        # needs sage.libs.pari
            False
    ...
    cpdef _floordiv_(self, right):
        ...
        EXAMPLES::
            ...
            sage: GF(7)(3) // 5                                                         # needs sage.rings.finite_rings
            2
        """
    ...
    def _vector_(self):
        ...
        EXAMPLES::
            ...
            sage: F.<a> = GF(13)                                                        # needs sage.rings.finite_rings
            sage: V = F.vector_space(map=False)                                         # needs sage.rings.finite_rings
            sage: V(a)                                                                  # needs sage.rings.finite_rings
            (1)
        """
...
cdef class IntegerMod_gmp(IntegerMod_abstract):
    ...
    def __pow__(IntegerMod_gmp self, exp, m): # NOTE: m ignored, always use modulus of parent ring
        ...
        We define ``0^0`` to be unity, :trac:`13894`::
            ...
            sage: p = next_prime(11^10)                                                 # needs sage.libs.pari
            sage: R = Integers(p)                                                       # needs sage.libs.pari
            sage: R(0)^0                                                                # needs sage.libs.pari
            1

        The value returned from ``0^0`` should belong to our ring::

            sage: type(R(0)^0) == type(R(0))                                            # needs sage.libs.pari
            True
...
cdef class IntegerMod_int(IntegerMod_abstract):
    ...
    def sqrt(self, extend=True, all=False):
        ...
        We compute all square roots in several cases::
            ...
            sage: R(0).sqrt(all=True)
            [0, 60, 120, 180, 240, 300]
            sage: GF(107)(0).sqrt(all=True)                                             # needs sage.rings.finite_rings
            [0]
        ...
        Check for :trac:`30797`::
            ...
            sage: GF(103)(-1).sqrt(extend=False, all=True)                              # needs sage.rings.finite_rings
            []
        """
...
def lucas_q1(mm, IntegerMod_abstract P):
    ...
    TESTS::
        ...
        sage: all(lucas_q1(k, a) == BinaryRecurrenceSequence(a, -1, 2, a)(k)            # needs sage.combinat
...
def lucas(k, P, Q=1, n=None):
    ...
    TESTS::
        ...
        sage: all(lucas(k, p, q, n)[0] == Mod(lucas_number2(k, p, q), n)                # needs sage.combinat
        ...
        sage: lucas(k, p, q, n) == [Mod(lucas_number2(k, p, q), n), Mod(q^(int(k/2)), n)]           # needs sage.combinat
        True
...
cdef class IntegerMod_to_Integer(Map):
    ...
    EXAMPLES::
        ...
        sage: ZZ.convert_map_from(GF(2))                                                # needs sage.rings.finite_rings
        Lifting map:
    ...
    def __init__(self, R):
        ...
        :trac:`15618`)::
            ...
            sage: ZZ.convert_map_from(GF(2)).parent()                                   # needs sage.rings.finite_rings
