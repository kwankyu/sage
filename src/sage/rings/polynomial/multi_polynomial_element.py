...
class MPolynomial_polydict(Polynomial_singular_repr, MPolynomial_element):
    ...
    def _macaulay2_(self, macaulay2=None):
        ...
        EXAMPLES::
            ...
            sage: R = GF(13)['a,b']['c,d']                                              # needs sage.rings.finite_rings
    ...
    def degree(self, x=None, std_grading=False):
        ...
        Degree of zero polynomial for other implementation :trac:`20048` ::
            ...
            sage: R.zero().degree(x)                                                    # needs sage.rings.finite_rings
            -1
        """
        if x is None:
    ...
    def coefficient(self, degrees):
        ...
        EXAMPLES::
            ...
            sage: R.<x, y> = QQbar[]                                                    # needs sage.rings.number_field
            sage: f = 2 * x * y                                                         # needs sage.rings.number_field
            sage: c = f.coefficient({x: 1, y: 1}); c                                    # needs sage.rings.number_field
            2
            sage: c.parent()                                                            # needs sage.rings.number_field
            Multivariate Polynomial Ring in x, y over Algebraic Field
            sage: c in PolynomialRing(QQbar, 2, names=['x', 'y'])                       # needs sage.rings.number_field
            True
            sage: f = y^2 - x^9 - 7*x + 5*x*y                                           # needs sage.rings.number_field
            sage: f.coefficient({y: 1})                                                 # needs sage.rings.number_field
            5*x
            sage: f.coefficient({y: 0})                                                 # needs sage.rings.number_field
            -x^9 + (-7)*x
            sage: f.coefficient({x: 0, y: 0})                                           # needs sage.rings.number_field
            0
            sage: f = (1+y+y^2) * (1+x+x^2)                                             # needs sage.rings.number_field
            sage: f.coefficient({x: 0})                                                 # needs sage.rings.number_field
            y^2 + y + 1
            sage: f.coefficient([0, None])                                              # needs sage.rings.number_field
            y^2 + y + 1
            sage: f.coefficient(x)                                                      # needs sage.rings.number_field
            y^2 + y + 1
            ...
            sage: f.coefficient(x^0) # outputs the full polynomial                      # needs sage.rings.number_field
    ...
    def lm(self):
        ...
        EXAMPLES::
            ...
            sage: R.<x,y,z> = PolynomialRing(GF(7), 3, order='lex')                     # needs sage.rings.finite_rings
            sage: (x^1*y^2 + y^3*z^4).lm()                                              # needs sage.rings.finite_rings
            x*y^2
            sage: (x^3*y^2*z^4 + x^3*y^2*z^1).lm()                                      # needs sage.rings.finite_rings
            x^3*y^2*z^4
        ...
        TESTS::
            ...
            sage: R.<x,y> = MPolynomialRing_polydict(GF(2), 2, order='lex')             # needs sage.rings.finite_rings
            sage: f = x + y                                                             # needs sage.rings.finite_rings
            sage: f.lm()                                                                # needs sage.rings.finite_rings
            x
