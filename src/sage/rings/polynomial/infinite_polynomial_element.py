...
permuting the indices of the variables::
    ...
    sage: P = Permutation(((4,5),(2,3)))                                                # needs sage.combinat
    sage: c^P                                                                           # needs sage.combinat
    ...
    def symmetric_cancellation_order(self, other):
        ...
        * w is 1 or is a term so that
        ...
        EXAMPLES::
            ...
            (None, 1, 1)
            sage: (x[2]*x[1]).symmetric_cancellation_order(x[2]*x[3]*y[1])              # needs sage.combinat
            (-1, [2, 3, 1], y_1)
            sage: (x[2]*x[1]*y[1]).symmetric_cancellation_order(x[2]*x[3]*y[1])         # needs sage.combinat
            (None, 1, 1)
            sage: (x[2]*x[1]*y[1]).symmetric_cancellation_order(x[2]*x[3]*y[2])         # needs sage.combinat
            (-1, [2, 3, 1], 1)
...
class InfinitePolynomial_sparse(InfinitePolynomial):
    ...
    def __pow__(self, n):
        ...
        EXAMPLES::
            ...
            sage: P = Permutation(((1,2),(3,4,5)))                                      # needs sage.combinat
            sage: p^P # indirect doctest                                                # needs sage.combinat
            x_10*y_1 + 2*x_2*y_4
