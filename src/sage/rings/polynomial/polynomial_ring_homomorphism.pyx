...
cdef class PolynomialRingHomomorphism_from_base(RingHomomorphism_from_base):
    ...
    cpdef Element _call_with_args(self, x, args=(), kwds={}):
        ...
        TESTS::
            ...
            sage: from sage.rings.polynomial.polynomial_ring_homomorphism import PolynomialRingHomomorphism_from_base
            sage: R.<x> = ZZ[]
            sage: S = GF(5)['x']                                                        # needs sage.rings.finite_rings
            sage: f = ZZ.hom(GF(5))                                                     # needs sage.rings.finite_rings
            sage: F = PolynomialRingHomomorphism_from_base(R.Hom(S), f)                 # needs sage.rings.finite_rings
            sage: F(2 * x, check=True)                                                  # needs sage.rings.finite_rings
            2*x
