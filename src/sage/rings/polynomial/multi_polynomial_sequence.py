...
def PolynomialSequence(arg1, arg2=None, immutable=False, cr=False, cr_str=None):
    ...
    EXAMPLES::
        ...
        sage: P.<a,b,c,d> = PolynomialRing(GF(127), 4)                                  # needs sage.rings.finite_rings
    ...
    We test that the ring is inferred correctly::
        ...
        sage: PolynomialSequence([1,x,y]).ring()                                        # needs sage.rings.finite_rings
        Multivariate Polynomial Ring in x, y, z over Finite Field of size 2

        sage: PolynomialSequence([[1,x,y], [0]]).ring()                                 # needs sage.rings.finite_rings
        Multivariate Polynomial Ring in x, y, z over Finite Field of size 2
    ...
    characteristic 2 (see :trac:`19452`)::
        ...
        sage: L.<t> = PowerSeriesRing(F,'t')                                            # needs sage.rings.finite_rings
        sage: R.<x,y> = PolynomialRing(L,'x,y')                                         # needs sage.rings.finite_rings
        sage: PolynomialSequence([0], R)                                                # needs sage.rings.finite_rings
        [0]
...
class PolynomialSequence_generic(Sequence_generic):
    ...
    def _repr_(self):
        ...
        EXAMPLES::
            ...
            sage: P.<a,b,c,d> = PolynomialRing(GF(127))                                 # needs sage.rings.finite_rings
            sage: I = sage.rings.ideal.Katsura(P)                                       # needs sage.rings.finite_rings
    ...
    def _groebner_strategy(self):
        ...
        EXAMPLES::
            ...
            sage: P.<x,y,z> = PolynomialRing(GF(127))                                   # needs sage.rings.finite_rings
            sage: F = Sequence([x*y + z, y + z + 1])                                    # needs sage.rings.finite_rings
    ...
    def __reduce__(self):
        ...
        TESTS::
            ...
            sage: P.<x,y,z> = PolynomialRing(GF(127))                                   # needs sage.rings.finite_rings
            sage: F = Sequence([x*y + z, y + z + 1])                                    # needs sage.rings.finite_rings
            sage: loads(dumps(F)) == F # indirect doctest                               # needs sage.rings.finite_rings
            True

        We check that :trac:`26354` is fixed::

            sage: f = P.hom([y,z,x])                                                    # needs sage.rings.finite_rings
            sage: hash(f) == hash(loads(dumps(f)))                                      # needs sage.rings.finite_rings
            True
    ...
    def solve(self, algorithm='polybori', n=1,  eliminate_linear_variables=True, verbose=False, **kwds):
        ...
        * ``verbose`` - whether to display progress and (potentially)
        ...
        Without argument, a single arbitrary solution is returned::
            ...
            sage: R.<x,y,z> = BooleanPolynomialRing()                                   # needs sage.rings.polynomial.pbori
            sage: S = Sequence([x*y + z, y*z + x, x + y + z + 1])                       # needs sage.rings.polynomial.pbori
            sage: sol = S.solve()                                                       # needs sage.rings.polynomial.pbori
            sage: print(reproducible_repr(sol))                                         # needs sage.rings.polynomial.pbori
            [{x: 0, y: 1, z: 0}]
