...
cdef class MPolynomial(CommutativePolynomial):
    ...
    def polynomial(self, var):
        ...
        EXAMPLES::
            ...
            sage: f = x^2 + x + y                                                       # needs sage.rings.finite_rings
            sage: f.polynomial(x)                                                       # needs sage.rings.finite_rings
            x^2 + x + y
            sage: f.polynomial(y)                                                       # needs sage.rings.finite_rings
            y + x^2 + x
        """
        cdef int ind
        R = self._parent
    ...
    def change_ring(self, R):
        ...
        ::
            ...
            sage: (x+2*y).change_ring(GF(3))                                            # needs sage.rings.finite_rings
            x - y
    ...
    def sylvester_matrix(self, right, variable = None):
        ...
        determinant of the Sylvester matrix is the resultant::
            ...
            sage: f.sylvester_matrix(g, x).determinant() == f.resultant(g, x)           # needs sage.modules
            True
        ...
        Polynomials must be defined over compatible base rings::
            ...
            [1 y]
            [1 y]
            sage: g.sylvester_matrix(h, 'x')                                            # needs sage.modules
            [1 y]
            [1 y]
            sage: f.sylvester_matrix(h, 'x')                                            # needs sage.modules
            Traceback (most recent call last):
            ...
    ...
    def macaulay_resultant(self, *args):
        ...
        an example when the coefficients live in a finite field::
            ...
            sage: F = FiniteField(11)                                                   # needs sage.rings.finite_rings
            sage: R.<x,y,z,w> = PolynomialRing(F, 4)                                    # needs sage.rings.finite_rings
    ...
    def denominator(self):
        ...
        has no denominator function. This closes :trac:`9063`::
            ...
            sage: R.<a,b,c> = GF(5)[]                                                   # needs sage.rings.finite_rings
            sage: x = R(0)                                                              # needs sage.rings.finite_rings
            sage: x.denominator()                                                       # needs sage.rings.finite_rings
            1
            sage: type(x.denominator())                                                 # needs sage.rings.finite_rings
            <class 'sage.rings.finite_rings.integer_mod.IntegerMod_int'>
            sage: type(a.denominator())                                                 # needs sage.rings.finite_rings
            <class 'sage.rings.finite_rings.integer_mod.IntegerMod_int'>
            sage: from sage.rings.polynomial.multi_polynomial_element import MPolynomial
            sage: isinstance(a / b, MPolynomial)                                        # needs sage.rings.finite_rings
            False
            sage: isinstance(a.numerator() / a.denominator(), MPolynomial)              # needs sage.rings.finite_rings
            True
        """
        if self.degree() == -1:
    ...
    def numerator(self):
        ...
        ::
            ...
            sage: K.<x,y,z> = GF(3)['x, y, z']                                          # needs sage.rings.finite_rings
            sage: f = 2*x*z + 2*z^2 + 2*y + 1; f                                        # needs sage.rings.finite_rings
            -x*z - z^2 - y + 1
            sage: f.numerator()                                                         # needs sage.rings.finite_rings
            -x*z - z^2 - y + 1
    ...
    def weighted_degree(self, *weights):
        ...
        ::
            ...
            sage: p.weighted_degree(-1, -2, -1)                                         # needs sage.rings.finite_rings
            -2
        ...
        ::
            ...
            sage: p.weighted_degree(x, 1, 1)                                            # needs sage.rings.finite_rings
            Traceback (most recent call last):
            ...
            TypeError: unable to convert non-constant polynomial x to Integer Ring
            sage: p.weighted_degree(2/1, 1, 1)                                          # needs sage.rings.finite_rings
            6
    ...
    def reduced_form(self, **kwds):
        ...
        ::
            ...
            sage: F.reduced_form(smallest_coeffs=False)  # tol 1e-11                    # needs sage.modules
            (
