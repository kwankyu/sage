...
def LaurentPolynomialRing(base_ring, *args, **kwds):
    ...
    1. ``LaurentPolynomialRing(base_ring, name, sparse=False)``
       ...
       You must specify a name::
           ...
           sage: R.<w> = LaurentPolynomialRing(PolynomialRing(GF(7),'k')); R            # needs sage.rings.finite_rings
    ...
    3. ``LaurentPolynomialRing(base_ring, name, n, order='degrevlex')``
       ...
       ::
           ...
           sage: LaurentPolynomialRing(GF(7), 'y', 5)                                   # needs sage.modules sage.rings.finite_rings
       ...
       method, all those variable names are available for interactive use::
           ...
           sage: R = LaurentPolynomialRing(GF(7), 15, 'w'); R                           # needs sage.modules sage.rings.finite_rings
           Multivariate Laurent Polynomial Ring in w0, w1, w2, w3, w4, w5, w6, w7,
            w8, w9, w10, w11, w12, w13, w14 over Finite Field of size 7
           sage: R.inject_variables()                                                   # needs sage.modules sage.rings.finite_rings
           Defining w0, w1, w2, w3, w4, w5, w6, w7, w8, w9, w10, w11, w12, w13, w14
           sage: (w0 + 2*w8 + w13)^2                                                    # needs sage.modules sage.rings.finite_rings
...
def from_fraction_field(L, x):
    ...
    EXAMPLES::
        ...
        sage: L.<x, y> = LaurentPolynomialRing(ZZ)                                      # needs sage.modules
        sage: F = L.fraction_field()                                                    # needs sage.modules
        sage: xi = F(~x)                                                                # needs sage.modules
        sage: from_fraction_field(L, xi) == ~x                                          # needs sage.modules
        True
    """
    d = L(x.denominator())
    if d.is_unit():
...
class LaurentPolynomialRing_mpair(LaurentPolynomialRing_generic):
    ...
    def _element_constructor_(self, x, mon=None):
        ...
        EXAMPLES::
            ...
            sage: var('x, y')                                                           # needs sage.modules
            (x, y)
            sage: M(x/y + 3/x)                                                          # needs sage.modules
            x*y^-1 + 3*x^-1

        ::

            sage: M(exp(x))                                                             # needs sage.modules
            Traceback (most recent call last):
            ...
            TypeError: unable to convert e^x to a rational
        ...
        ::
            ...
            sage: R.<x,y,z> = LaurentPolynomialRing(QQ)                                 # needs sage.modules
            sage: mon = ETuple({}, int(3))                                              # needs sage.modules
            sage: P = R.polynomial_ring()                                               # needs sage.modules
            sage: R(sum(P.gens()), mon)                                                 # needs sage.modules
            x + y + z
            sage: R(sum(P.gens()), (-1,-1,-1))                                          # needs sage.modules
