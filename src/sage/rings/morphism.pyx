...
Map from single variable polynomial ring::
    ...
    sage: R.<x> = ZZ[]
    sage: phi = R.hom([2], GF(5))                                                       # needs sage.rings.finite_rings
    sage: phi                                                                           # needs sage.rings.finite_rings
    Ring morphism:
      From: Univariate Polynomial Ring in x over Integer Ring
      To:   Finite Field of size 5
      Defn: x |--> 2
    sage: phi(x + 12)                                                                   # needs sage.rings.finite_rings
    4
...
::
    ...
    sage: c == loads(dumps(c))                                                          # needs sage.rings.number_field
    True

::

    sage: R.<t> = PowerSeriesRing(GF(5))                                                # needs sage.rings.finite_rings
    sage: f = R.hom([t^5])                                                              # needs sage.rings.finite_rings
...
compare equal::
    ...
    sage: phi1 = H.identity(); phi1                                                     # needs sage.rings.finite_rings
    Identity endomorphism of Univariate Quotient Polynomial Ring in a
     over Finite Field of size 2 with modulus x^2 + x + 1
    sage: phi2 = H([a]); phi2                                                           # needs sage.rings.finite_rings
    Ring endomorphism of Univariate Quotient Polynomial Ring in a
     over Finite Field of size 2 with modulus x^2 + x + 1
      Defn: a |--> a
    sage: phi3 = RingHomomorphism_from_base(H, R.hom([x])); phi3                        # needs sage.rings.finite_rings
    Ring endomorphism of Univariate Quotient Polynomial Ring in a
     over Finite Field of size 2 with modulus x^2 + x + 1
      Defn: Induced from base ring by
            Ring endomorphism of Univariate Polynomial Ring in x
             over Finite Field of size 2 (using GF2X)
              Defn: x |--> x
    sage: phi4 = RingHomomorphism_cover(H); phi4                                        # needs sage.rings.finite_rings
    Ring endomorphism of Univariate Quotient Polynomial Ring in a
     over Finite Field of size 2 with modulus x^2 + x + 1
      Defn: Natural quotient map
    sage: phi5 = F4.frobenius_endomorphism() ^ 2; phi5                                  # needs sage.rings.finite_rings
    Frobenius endomorphism x |--> x^(2^2) of
     Univariate Quotient Polynomial Ring in a
     over Finite Field of size 2 with modulus x^2 + x + 1
    sage: maps = [phi1, phi2, phi3, phi4, phi5]                                         # needs sage.rings.finite_rings
    sage: for f in maps:                                                                # needs sage.rings.finite_rings
    ....:     for g in maps:
    ....:         if f != g:
    ....:             print("{} != {}".format(f, g))
"""
...
cdef class RingMap_lift(RingMap):
    ...
    def __init__(self, R, S):
        ...
        An invalid example::
            ...
            sage: from sage.rings.morphism import RingMap_lift                          # needs sage.rings.number_field
...
cdef class RingHomomorphism(RingMap):
    ...
    def kernel(self):
        ...
        The Steiner-Roman surface::
            ...
            sage: R.<x,y,z> = QQ[]
            sage: S = R.quotient(x^2 + y^2 + z^2 - 1)                                   # needs sage.libs.singular
    ...
    def inverse(self):
        ...
        Check that results are cached::
            ...
            sage: R.<x,y> = GF(823)[]                                                   # needs sage.rings.finite_rings
            sage: f = R.hom([x, y+x^2], R)                                              # needs sage.rings.finite_rings
        ...
        Some subclasses of ring homomorphisms are not supported::
            ...
            sage: K.<u> = PowerSeriesRing(GF(5))                                        # needs sage.rings.finite_rings
            sage: FrobeniusEndomorphism_generic(K).inverse()                            # needs sage.rings.finite_rings
            Traceback (most recent call last):
            ...
            NotImplementedError
    ...
    def __invert__(self):
        ...
        EXAMPLES::
            ...
            sage: R.<x,y> = GF(17)[]                                                    # needs sage.rings.finite_rings
            sage: f = R.hom([3*x, y + x^2 + x^3], R)                                    # needs sage.rings.finite_rings
...
cdef class RingHomomorphism_im_gens(RingHomomorphism):
    ...
    cpdef _richcmp_(self, other, int op):
        ...
        ::
            ...
            sage: R.<x,y> = QQ[]; f = R.hom([x, x + y]); g = R.hom([y, x])              # needs sage.libs.pari
            sage: f == g             # indirect doctest                                 # needs sage.libs.pari
            False
...
cdef class RingHomomorphism_from_base(RingHomomorphism):
    ...
    cpdef _richcmp_(self, other, int op):
        ...
        `\QQ`::
            ...
            sage: PR.<s,t> = R[]                                                        # needs sage.libs.pari sage.libs.singular sage.modules
    ...
    def inverse(self):
        ...
        EXAMPLES::
            ...
            sage: R.<x,y> = QQ[]
            sage: S.<a,b> = QQ[]
            sage: f = R.hom([a + b, a - b], S)                                          # needs sage.libs.singular
            sage: PR.<t> = R[]
            sage: PS = S['t']
            sage: Pf = PR.hom(f, PS)                                                    # needs sage.libs.singular
...
cdef class FrobeniusEndomorphism_generic(RingHomomorphism):
    ...
    def __init__(self, domain, n=1):
        ...
        TESTS::
            ...
            sage: from sage.rings.morphism import FrobeniusEndomorphism_generic
            sage: K.<u> = PowerSeriesRing(GF(5))                                        # needs sage.rings.finite_rings
            sage: FrobeniusEndomorphism_generic(K)                                      # needs sage.rings.finite_rings
            Frobenius endomorphism x |--> x^5 of Power Series Ring in u
             over Finite Field of size 5
            sage: FrobeniusEndomorphism_generic(K, 2)                                   # needs sage.rings.finite_rings
    ...
    def _repr_(self):
        ...
        EXAMPLES::
            ...
            sage: K.<u> = PowerSeriesRing(GF(5))                                        # needs sage.rings.finite_rings
            sage: Frob = K.frobenius_endomorphism(); Frob                               # needs sage.rings.finite_rings
            Frobenius endomorphism x |--> x^5 of Power Series Ring in u
             over Finite Field of size 5

            sage: Frob._repr_()                                                         # needs sage.rings.finite_rings
            'Frobenius endomorphism x |--> x^5 of Power Series Ring in u
             over Finite Field of size 5'
        """
        if self._power == 0:
    ...
    def _latex_(self):
        ...
        EXAMPLES::
            ...
            sage: K.<u> = PowerSeriesRing(GF(5))                                        # needs sage.rings.finite_rings
            sage: Frob = K.frobenius_endomorphism(2)                                    # needs sage.rings.finite_rings
            sage: Frob._latex_()                                                        # needs sage.rings.finite_rings
            '\\verb"Frob"^{2}'
        """
        if self._power == 0:
            s = '\\verb"id"'
    ...
    def __pow__(self, n, ignored):
        ...
        EXAMPLES::
            ...
            Frobenius endomorphism x |--> x^5 of Power Series Ring in u
             over Finite Field of size 5
            sage: Frob^2                                                                # needs sage.rings.finite_rings
            Frobenius endomorphism x |--> x^(5^2) of Power Series Ring in u
             over Finite Field of size 5
        """
