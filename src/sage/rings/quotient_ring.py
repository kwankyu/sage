...
easily::
    ...
    sage: Q3 = F.quo(F*[F.prod(m) for m in product(F.gens(), repeat=3)]*F)              # needs sage.combinat sage.modules
    sage: Q3                                                                            # needs sage.combinat sage.modules
    ...
    sage: Q3.0*Q3.1 - Q3.1*Q3.0                                                         # needs sage.combinat sage.modules
    xbar*ybar - ybar*xbar
    sage: Q3.0*(Q3.1*Q3.2) - (Q3.1*Q3.2)*Q3.0                                           # needs sage.combinat sage.modules
    0
    sage: Q2 = F.quo(F*[F.prod(m) for m in product(F.gens(), repeat=2)]*F)              # needs sage.combinat sage.modules
    sage: Q2.is_commutative()                                                           # needs sage.combinat sage.modules
    True
...
class QuotientRing_nc(ring.Ring, sage.structure.parent_gens.ParentWithGens):
    ...
    def construction(self):
        ...
        EXAMPLES::
            ...
            sage: F.<x,y,z> = FreeAlgebra(QQ, implementation='letterplace')             # needs sage.combinat sage.modules
            sage: I = F * [x*y + y*z, x^2 + x*y - y*x - y^2] * F                        # needs sage.combinat sage.modules
            sage: Q = F.quo(I)                                                          # needs sage.combinat sage.modules
            sage: Q.construction()                                                      # needs sage.combinat sage.modules
            (QuotientFunctor,
             Free Associative Unital Algebra on 3 generators (x, y, z) over Rational Field)
        ...
        TESTS::
            ...
            sage: F, R = GF(5).construction()                                           # needs sage.rings.finite_rings
    ...
    def is_commutative(self):
        ...
        In the next example, the generators apparently commute::
            ...
            sage: J = F * [x*y - y*x, x*z - z*x, y*z - z*y, x^3 - y^3] * F              # needs sage.combinat sage.modules
            sage: R = F.quo(J)                                                          # needs sage.combinat sage.modules
            sage: R.is_commutative()                                                    # needs sage.combinat sage.modules
            True
    ...
    def is_field(self, proof=True):
        ...
        TESTS::
            ...
            sage: Q = QuotientRing(ZZ, 7*ZZ)
            sage: Q.is_field()                                                          # needs sage.libs.pari
            True
        ...
        implemented::
            ...
            sage: R.<x, y> = ZZ[]
            sage: R.quotient_ring(R.ideal([2, 4 + x])).is_field()                       # needs sage.libs.pari
            Traceback (most recent call last):
            ...
            NotImplementedError
        """
    ...
    def is_integral_domain(self, proof=True):
        ...
        EXAMPLES::
            ...
            False
            sage: R.<a,b,c> = ZZ[]                                                      # needs sage.singular
            sage: Q = R.quotient_ring([a, b])                                           # needs sage.singular
            sage: Q.is_integral_domain()                                                # needs sage.singular
            Traceback (most recent call last):
            ...
            NotImplementedError
            sage: Q.is_integral_domain(proof=False)                                     # needs sage.singular
            False
        """
        if proof:
    ...
    def is_noetherian(self):
        ...
        EXAMPLES::
            ...
            sage: R.is_noetherian()                                                     # needs sage.libs.pari
            True
        ...
        raise an error::
            ...
            sage: S = R.quotient(I)                                                     # needs sage.libs.pari
            sage: S.is_noetherian()                                                     # needs sage.libs.pari
            Traceback (most recent call last):
            ...
            NotImplementedError
        """
    ...
    def _element_constructor_(self, x, coerce=True):
        ...
        ::
            ...
            -b^2 - b
            sage: S.coerce(GF(7)(3))                                                    # needs sage.libs.singular sage.rings.finite_rings
            Traceback (most recent call last):
            ...
    ...
    def _coerce_map_from_(self, R):
        ...
        We check that :trac:`13682` is fixed::
            ...
            True
            sage: S = R.quotient(I)                                                     # needs sage.libs.singular
            sage: T = R.quotient(J)                                                     # needs sage.libs.singular
        ...
        We also allow coercions with the cover rings::
            ...
            sage: Sp = Rp.quotient(Ip)                                                  # needs sage.libs.singular
            sage: Tp = Rp.quotient(Jp)                                                  # needs sage.libs.singular
            sage: R.has_coerce_map_from(Rp)                                             # needs sage.libs.singular
            True
            sage: Sp.has_coerce_map_from(Sp)                                            # needs sage.libs.singular
            True
    ...
    def __richcmp__(self, other, op):
        ...
        quotient rings are not equal::
            ...
            True
            sage: R.quotient_ring(x^2 + y^2) == R.quotient_ring(-x^2 - y^2)             # needs sage.libs.singular
            False
        """
    ...
    def gen(self, i=0):
        ...
        ::
            ...
            Quotient of Multivariate Polynomial Ring in x, y over Rational Field
             by the ideal (x, y^2 + 1)
            sage: R.gen(0); R.gen(1)                                                    # needs sage.libs.singular
            x
            y
    ...
    def _singular_init_(self, singular=None):
        ...
        EXAMPLES::
            ...
            sage: parent(S)                                                             # needs sage.libs.singular
    ...
    def _magma_init_(self, magma):
        ...
        EXAMPLES::
            ...
            sage: P.<x,y> = PolynomialRing(GF(2))                                       # needs sage.rings.finite_rings
            sage: Q = P.quotient(sage.rings.ideal.FieldIdeal(P))                        # needs sage.rings.finite_rings
