...
cdef class FractionFieldElement(FieldElement):
    ...
    def __init__(self, parent, numerator, denominator=1,
        ...
        EXAMPLES::
            ...
            sage: K((x + 1)/(x^2 + x + 1))                                              # needs sage.symbolic
            (x + 1)/(x^2 + x + 1)
            sage: K(355/113)
            355/113
    ...
    def _im_gens_(self, codomain, im_gens, base_map=None):
        ...
        EXAMPLES::
            ...
            sage: x,y = F.gens()
            sage: K = GF(7)['a,b'].fraction_field()                                     # needs sage.rings.finite_rings
            sage: a,b = K.gens()                                                        # needs sage.rings.finite_rings

        ::

            sage: phi = F.hom([a + b, a*b], K)                                          # needs sage.rings.finite_rings
            sage: phi(x+y) # indirect doctest                                           # needs sage.rings.finite_rings
            a*b + a + b

        ::

            sage: (x^2/y)._im_gens_(K, [a + b, a*b])                                    # needs sage.rings.finite_rings
            (a^2 + 2*a*b + b^2)/(a*b)
            sage: (x^2/y)._im_gens_(K, [a, a*b])                                        # needs sage.rings.finite_rings
            a/b
    ...
    def __hash__(self):
        ...
        Check that :trac:`25199` is fixed::
            ...
            sage: hash(R.0) == hash(FractionField(R).0)                                 # needs sage.rings.number_field
            True
            sage: ((x+1)/(x^2+1)).subs({x: 1})                                          # needs sage.rings.number_field
            1
        """
    ...
    cpdef _add_(self, right):
        ...
        Subtraction is implemented by adding the negative::
            ...
            sage: K.<t> = Frac(GF(7)['t'])                                              # needs sage.rings.finite_rings
            sage: t - 1/t # indirect doctest                                            # needs sage.rings.finite_rings
            (t^2 + 6)/t
        """
    ...
    def __neg__(self):
        ...
        EXAMPLES::
            ...
            sage: K.<t> = Frac(GF(5)['t'])                                              # needs sage.rings.finite_rings
            sage: f = (t^2+t)/(t+2); f                                                  # needs sage.rings.finite_rings
            (t^2 + t)/(t + 2)
    ...
    def __invert__(self):
        ...
        EXAMPLES::
            ...
            sage: K.<t> = Frac(GF(7)['t'])                                              # needs sage.rings.finite_rings
            sage: f = (t^2+5)/(t-1)                                                     # needs sage.rings.finite_rings
            sage: ~f                                                                    # needs sage.rings.finite_rings
            (t + 6)/(t^2 + 5)
        """
        if self.is_zero():
    ...
    def _evaluate_polynomial(self, pol):
        ...
        Check that :trac:`25440` has been resolved::
            ...
            sage: S.<y> = R.fraction_field()[]                                          # needs sage.rings.finite_rings
            sage: (y+1)(R.one())                                                        # needs sage.rings.finite_rings
            0
