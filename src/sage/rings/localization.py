...
Check relations of the Ariki-Koike algebra::
    ...
    sage: m1*m2*m1*m2 == m2*m1*m2*m1                                                    # needs sage.libs.pari
    True
    sage: m2*m3*m2 == m3*m2*m3                                                          # needs sage.libs.pari
    True
    sage: m1*m3 == m3*m1                                                                # needs sage.libs.pari
    True
    sage: m1**3 - (u0+u1+u2)*m1**2 + (u0*u1+u0*u2+u1*u2)*m1 - u0*u1*u2 == 0             # needs sage.libs.pari
    True
    sage: m2**2 - (q-1)*m2 - q == 0                                                     # needs sage.libs.pari
    True
    sage: m3**2 - (q-1)*m3 - q == 0                                                     # needs sage.libs.pari
    True
    sage: ~m1 in m1.parent()                                                            # needs sage.libs.pari
    True
    sage: ~m2 in m2.parent()                                                            # needs sage.libs.pari
    True
    sage: ~m3 in m3.parent()                                                            # needs sage.libs.pari
    True
...
Obtain specializations in positive characteristic::
    ...
    [0 3 0]
    [0 0 3]
    sage: mFp1.base_ring()                                                              # needs sage.modules
...
Obtain specializations in characteristic 0::
    ...
    sage: # needs sage.rings.finite_rings
    sage: fQ = L.hom((3,5,7,11), codomain=QQ); fQ                                       # needs sage.libs.pari
    Ring morphism:
      ...
      Defn: u0 |--> 3
            ...
            u2 |--> 7
            q |--> 11
    sage: mQ1 = matrix({k: fQ(v) for k, v in m1.dict().items()}); mQ1                   # needs sage.libs.pari sage.modules
    [5 0 0]
    [0 3 0]
    [0 0 3]
    sage: mQ1.base_ring()                                                               # needs sage.modules
    Rational Field
    sage: mQ2 = matrix({k: fQ(v) for k, v in m2.dict().items()}); mQ2                   # needs sage.libs.pari sage.modules
    [-15 -14   0]
    [ 26  25   0]
    [  0   0  -1]
    sage: mQ3 = matrix({k: fQ(v) for k, v in m3.dict().items()}); mQ3                   # needs sage.libs.pari sage.modules
    [    -1      0      0]
    [     0 -15/26  11/26]
    [     0 301/26 275/26]
    ...
    sage: S.<x, y, z, t> = QQ[]
    sage: T = S.quo(x + y + z)
    sage: F = T.fraction_field()                                                        # needs sage.libs.singular
    ...
    [           0 -ybar - zbar            0]
    [           0            0 -ybar - zbar]
    sage: mF1.base_ring() == F                                                          # needs sage.libs.singular sage.modules
    True
...
def normalize_extra_units(base_ring, add_units, warning=True):
    ...
    EXAMPLES::
        ...
        [2, 3, 5]
        sage: P.<x,y,z> = ZZ[]
        sage: normalize_extra_units(P, [3*x, z*y**2, 2*z, 18*(x*y*z)**2, x*z, 6*x*z, 5])            # needs sage.libs.pari
        [2, 3, 5, z, y, x]
        sage: P.<x,y,z> = QQ[]
        sage: normalize_extra_units(P, [3*x, z*y**2, 2*z, 18*(x*y*z)**2, x*z, 6*x*z, 5])            # needs sage.libs.pari
        [z, y, x]
...
class LocalizationElement(IntegralDomainElement):
    ...
    EXAMPLES::
        ...
        sage: P.<x,y,z> = GF(5)[]                                                       # needs sage.rings.finite_rings
        sage: L = P.localization((x, y*z-x))                                            # needs sage.rings.finite_rings
        sage: LocalizationElement(L, 4/(y*z-x)**2)                                      # needs sage.rings.finite_rings
        (-1)/(y^2*z^2 - 2*x*y*z + x^2)
        sage: _.parent()                                                                # needs sage.rings.finite_rings
    ...
    def _rational_(self):
        ...
        TESTS::
            ...
            sage: L = ZZ.localization(5)
            sage: cp3 = cyclotomic_polynomial(3).change_ring(L)                         # needs sage.libs.pari
...
class Localization(IntegralDomain, UniqueRepresentation):
    ...
    EXAMPLES::
        ...
        True
        sage: t = (x+5)/(x**2+2)
        sage: t in L                                                                    # needs sage.libs.pari
        False
    ...
    def _repr_(self):
        ...
        EXAMPLES::
            ...
            sage: R.<a> = GF(3)[]                                                       # needs sage.rings.finite_rings
    ...
    def characteristic(self):
        ...
        EXAMPLES::
            ...
            sage: R.<a> = GF(5)[]                                                       # needs sage.rings.finite_rings
