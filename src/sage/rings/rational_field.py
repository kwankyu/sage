...
class RationalField(Singleton, number_field_base.NumberField):
    ...
    def primes_of_bounded_norm_iter(self, B):
        ...
        EXAMPLES::
            ...
            [2, 3, 5, 7]
            sage: list(QQ.primes_of_bounded_norm_iter(1))                               # needs sage.libs.pari
            []
        """
        try:
            B = ZZ(B.ceil())
    ...
    def embeddings(self, K):
        ...
        `K` must have characteristic 0::
            ...
            sage: QQ.embeddings(GF(3))                                                  # needs sage.rings.finite_rings
            Traceback (most recent call last):
            ...
    ...
    def hilbert_symbol_negative_at_S(self, S, b, check=True):
        ...
        ::
            ...
            sage: QQ.hilbert_symbol_negative_at_S([1, 3], sqrt(2))                      # needs sage.libs.pari sage.modules
            Traceback (most recent call last):
            ...
    ...
    def selmer_space(self, S, p, proof=None):
        ...
        (in this case, modulo squares)::
            ...
            sage: toQS2(-5/7)                                                           # needs sage.modules
            (1, 1, 1)
            sage: fromQS2((1,1,1))                                                      # needs sage.modules
            -35
            sage: ((-5/7)/(-35)).is_square()                                            # needs sage.modules
            True

        The map ``toQS2`` is not defined on all of `\QQ^*`, only on
        those numbers which are squares away from `5` and `7`::

            sage: toQS2(210)                                                            # needs sage.modules
            Traceback (most recent call last):
            ...
