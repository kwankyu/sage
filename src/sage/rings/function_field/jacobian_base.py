...
is automatically chosen if not given. ::
    ...
    sage: J = F.jacobian()
    sage: J
    Jacobian of Function field in z defined by z^3 + 23*y^2*z + 6
    over Finite Field of size 29 (Hess model)
...
Explicitly specify a model if you want Jacobians in different models. ::
    ...
    sage: J_km
    Jacobian of Function field in z defined by z^3 + 23*y^2*z + 6
    over Finite Field of size 29 (Khuri-Makdisi large model)
...
class JacobianGroupFunctor(ConstructionFunctor):
    ...
    def merge(self, other):
        ...
        EXAMPLES::
            ...
            sage: G3 = J.group(K3)
            sage: sage.categories.pushout.pushout(G2, G3)
...
class Jacobian_base(Parent):
    ...
    EXAMPLES::
        ...
        sage: F.jacobian()
        Jacobian of Function field in y defined by y^2 + y + (x^2 + 1)/x
        over Finite Field of size 2 (Hess model)
    """
    def __init__(self, function_field, base_div):
        """
        Initialize.
        ...
        """
        super().__init__(category=Jacobians(), facade=True)
    ...
    def _repr_(self):
        ...
        """
        F = self._function_field
        return f'Jacobian of {F} over {F.constant_base_field()}'
    ...
    def __call__(self, x):
        ...
        TESTS::
            ...
            sage: G_hess = J_hess.group()
            sage: p = G_hess.get_points(3)[2]
            sage: J_km = F.jacobian(model='km_large')
            sage: q = J_km(p)
        ...
        if isinstance(x, JacobianPoint_base):
            Gx = x.parent()
            Jx = Gx._parent
            if Jx._function_field is self._function_field:
                k = Gx._function_field.constant_base_field()
                G = self.group(k)
                F = G._function_field
                return G.point(F.divisor_group()(x.divisor()))
        raise ValueError(f"Cannot create a point of the Jacobian from {x}")

    def facade_for(self):
        """
        Return a parent that this Jacobian is a facade for.

        Only one parent, the group of rational points, is returned, even though
    ...
    def set_base_place(self, place):
        ...
        field to the point of the Jacobian defined by `P - B`.
