...
def QuaternionMatrixGroupGF3():
    ...
    is the product of `I` and `J`. ::
        ...
        sage: Q = QuaternionMatrixGroupGF3()                                            # needs sage.rings.finite_rings
        sage: Q.order()                                                                 # needs sage.rings.finite_rings
        8
        sage: aye = Q.gens()[0]; aye                                                    # needs sage.rings.finite_rings
        [1 1]
        [1 2]
        sage: jay = Q.gens()[1]; jay                                                    # needs sage.rings.finite_rings
        [2 1]
        [1 1]
        sage: kay = aye*jay; kay                                                        # needs sage.rings.finite_rings
        [0 2]
        [1 0]
...
def MatrixGroup(*gens, **kwds):
    ...
    EXAMPLES::
        ...
        sage: gens = [matrix(F, 2, [1,2, -1,1]), matrix(F,2, [1,1, 0,1])]               # needs sage.rings.finite_rings
        sage: G = MatrixGroup(gens); G                                                  # needs sage.rings.finite_rings
    ...
    there::
        ...
        sage: gens = [matrix(2, [1,2, -1,1]), matrix(GF(7), 2, [1,1, 0,1]), 2]          # needs sage.rings.finite_rings
        sage: G = MatrixGroup(gens); G                                                  # needs sage.rings.finite_rings
    ...
    Each generator must be invertible::
        ...
        sage: F = GF(5); MS = MatrixSpace(F, 2, 2)                                      # needs sage.rings.finite_rings
        sage: MatrixGroup([MS.0])                                                       # needs sage.rings.finite_rings
        Traceback (most recent call last):
        ...
        ValueError: each generator must be an invertible matrix
        sage: MatrixGroup([MS.0], check=False)  # works formally but is mathematical nonsense       # needs sage.rings.finite_rings
...
class FinitelyGeneratedMatrixGroup_generic(MatrixGroup_generic):
    ...
    TESTS::
        ...
        sage: G = GL(2, GF(3))                                                          # needs sage.rings.finite_rings
