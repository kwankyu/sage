...
cdef inline wrapperbase* get_slotdef(wrapper_descriptor slotwrapper) except NULL:
    ...
    TESTS::
        ...
        ....: ''')
        sage: py_get_slotdef(object.__init__)  # random                                 # needs sage.misc.cython
        140016903442416
        sage: py_get_slotdef(bytes.__lt__)  # random                                    # needs sage.misc.cython
        140016903441800
        sage: py_get_slotdef(bytes.__lt__) == py_get_slotdef(Integer.__lt__)            # needs sage.misc.cython
        True
        sage: py_get_slotdef(bytes.__lt__) == py_get_slotdef(bytes.__gt__)              # needs sage.misc.cython
        False
        sage: class X():
        ....:     def __eq__(self, other):
        ....:         return False
        sage: py_get_slotdef(X.__eq__)                                                  # needs sage.misc.cython
