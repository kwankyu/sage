...
class AbstractSimplex_class(SageObject):
    ...
    def __deepcopy__(self, memo):
        ...
        copies of simplicial sets::
            ...
            sage: dict(copy.copy(RP3._data)) == dict(RP3._data)                         # needs sage.groups
            True
            sage: dict(copy.deepcopy(RP3._data)) == dict(RP3._data)                     # needs sage.groups
            False
            sage: SimplicialSet(RP3) == RP3                                             # needs sage.groups
            False
            sage: copy.copy(RP3) == RP3                                                 # needs sage.groups
            False
        """
        underlying = self.nondegenerate()
    ...
    *n-simplices*, indexed by the non-negative integers, together with
    ...
    def cells(self, subcomplex=None, max_dim=None):
        ...
        EXAMPLES::
            ...
            sage: S0.cells(S0.subsimplicial_set([v, w]))                                # needs sage.graphs
            {0: [*]}

            sage: X = SimplicialSet({e: (v,w)})
            sage: X.cells(X.subsimplicial_set([v, w]))                                  # needs sage.graphs
            {0: [*], 1: [e]}
    ...
    def graph(self):
        ...
        EXAMPLES::
            ...
            sage: G = Delta3.graph()                                                    # needs sage.graphs
            ...
            sage: T = simplicial_sets.Torus()
            sage: T.graph()                                                             # needs sage.graphs
            Looped multi-graph on 1 vertex
            sage: len(T.graph().edges(sort=False))                                      # needs sage.graphs
            3

            sage: CP3 = simplicial_sets.ComplexProjectiveSpace(3)
            sage: G = CP3.graph()                                                       # needs sage.graphs
            sage: len(G.vertices(sort=False))                                           # needs sage.graphs
            1
            sage: len(G.edges(sort=False))                                              # needs sage.graphs
            0

            sage: Sigma3 = groups.permutation.Symmetric(3)                              # needs sage.groups
            sage: Sigma3.nerve().is_connected()                                         # needs sage.graphs sage.groups
            True
        """
    ...
    def is_connected(self):
        ...
        EXAMPLES::
            ...
            sage: T.is_connected()                                                      # needs sage.graphs
            True
            sage: K.is_connected()                                                      # needs sage.graphs
            True
            sage: X.is_connected()                                                      # needs sage.graphs
            False
            sage: simplicial_sets.Sphere(0).is_connected()                              # needs sage.graphs
            False
        """
    ...
    def chain_complex(self, dimensions=None, base_ring=ZZ, augmented=False,
        ...
        EXAMPLES::
            ...
            sage: C3 = groups.misc.MultiplicativeAbelian([3])                           # needs sage.groups
            sage: BC3 = simplicial_sets.ClassifyingSpace(C3)                            # needs sage.groups
            sage: BC3.chain_complex(range(4), base_ring=GF(3))                          # needs sage.groups sage.rings.finite_rings
    ...
    def n_chains(self, n, base_ring=ZZ, cochains=False):
        ...
        EXAMPLES::
            ...
            sage: list(BSigma3.n_chains(1).basis())                                     # needs sage.groups
            [(1,2), (1,2,3), (1,3), (1,3,2), (2,3)]
            sage: list(BSigma3.n_chains(1, cochains=True).basis())                      # needs sage.groups
    ...
    def quotient(self, subcomplex, vertex_name='*'):
        ...
        EXAMPLES::
            ...
            sage: Y = X.quotient([f])                                                   # needs sage.graphs
            sage: Y.nondegenerate_simplices()                                           # needs sage.graphs
            [*, e]
            sage: Y.homology(1)                                                         # needs sage.graphs
            Z

            sage: E = SimplicialSet({e: (v, w)})
            sage: Z = E.quotient([v, w])                                                # needs sage.graphs
            sage: Z.nondegenerate_simplices()                                           # needs sage.graphs
            [*, e]
            sage: Z.homology(1)                                                         # needs sage.graphs
            Z

            sage: F = E.quotient([v])                                                   # needs sage.graphs
            sage: F.nondegenerate_simplices()                                           # needs sage.graphs
            [*, w, e]
            sage: F.base_point()                                                        # needs sage.graphs
            *

            sage: RP5 = simplicial_sets.RealProjectiveSpace(5)
            sage: RP2 = RP5.n_skeleton(2)
            sage: RP5_2 = RP5.quotient(RP2)                                             # needs sage.graphs
            sage: RP5_2.homology(base_ring=GF(2))                                       # needs sage.graphs sage.modules sage.rings.finite_rings
        ...
        Behavior of base points::
            ...
            [(2,), (3,), (2, 3)]
            sage: K.quotient([K.n_cells(1)[-1]]).base_point()                           # needs sage.graphs
            *
            ...
            sage: L.nondegenerate_simplices()
            [(2,), (3,), (2, 3)]
            sage: K.quotient(L).base_point()                                            # needs sage.graphs
            (0,)

        TESTS::

            sage: pt = RP5.quotient(RP5.n_skeleton(5)); pt                              # needs sage.graphs sage.groups
            Quotient: (RP^5/RP^5)
            sage: len(pt.nondegenerate_simplices())                                     # needs sage.graphs sage.groups
            1
        """
        from .simplicial_set_constructions import SubSimplicialSet
    ...
    def disjoint_union(self, *others):
        ...
        EXAMPLES::
            ...
            sage: Z = X.disjoint_union(Y)                                               # needs sage.graphs
        ...
        Factors and inclusion maps::
            ...
            sage: T = simplicial_sets.Torus()
            sage: S2 = simplicial_sets.Sphere(2)
            sage: A = T.disjoint_union(S2)                                              # needs sage.graphs
            sage: A.factors()                                                           # needs sage.graphs
            (Torus, S^2)
            sage: i = A.inclusion_map(0)                                                # needs sage.graphs
            sage: i.domain()                                                            # needs sage.graphs
            Torus
            sage: i.codomain()                                                          # needs sage.graphs
        ...
        Empty factors are ignored::
            ...
            sage: from sage.topology.simplicial_set_examples import Empty
            sage: E = Empty()
            sage: K = S2.disjoint_union(S2, E, E, S2)                                   # needs sage.graphs
            sage: K == S2.disjoint_union(S2, S2)                                        # needs sage.graphs
            True
            sage: K.factors()                                                           # needs sage.graphs
            (S^2, S^2, S^2)
        """
    ...
    def coproduct(self, *others):
        ...
        summand, as long as the summands are all finite::
            ...
            sage: S2.coproduct(K).inclusion_map(0)                                      # needs sage.graphs
            Simplicial set morphism:
              From: S^2
              To:   Wedge: (S^2 v Klein bottle)
              Defn: [v_0, sigma_2] --> [*, sigma_2]
            sage: D3.coproduct(Y, Z).inclusion_map(2)                                   # needs sage.graphs
            Simplicial set morphism:
        ...
        TESTS::
            ...
            sage: D3.coproduct(S2, Z)                                                   # needs sage.graphs
            Traceback (most recent call last):
            ...
    ...
    def pushout(self, *maps):
        ...
        Construct the 4-sphere as a quotient of a 4-simplex::
            ...
            sage: S4 = L.pushout(L.constant_map(), L.inclusion_map()); S4               # needs sage.graphs
            Pushout of maps:
              Simplicial set morphism:
            ...
            sage: len(S4.nondegenerate_simplices())                                     # needs sage.graphs
            2
            sage: S4.homology(4)                                                        # needs sage.graphs sage.modules
            Z
        ...
        The associated maps::
            ...
            sage: W = S1.wedge(T)  # wedge, constructed as a pushout                    # needs sage.graphs
            sage: W.defining_map(1)                                                     # needs sage.graphs
            Simplicial set morphism:
              From: Point
              To:   S^1 x S^1
              Defn: Constant map at (v_0, v_0)
            sage: W.structure_map(0)                                                    # needs sage.graphs
            Simplicial set morphism:
              From: S^1
              To:   Wedge: (S^1 v S^1 x S^1)
              Defn: [v_0, sigma_1] --> [*, sigma_1]

            sage: f = S1.Hom(T)({S1.n_cells(0)[0]: K.n_cells(0)[0],                     # needs sage.graphs sage.modules
            ....:                S1.n_cells(1)[0]: K.n_cells(1)[0]})

        The maps `f: S^1 \to T` and `1: T \to T` induce a map `S^1 \vee T \to T`::

            sage: g = W.universal_property(f, Hom(T,T).identity())                      # needs sage.graphs sage.modules
            sage: g.domain() == W                                                       # needs sage.graphs sage.modules
            True
            sage: g.codomain() == T                                                     # needs sage.graphs sage.modules
            True

        TESTS::

            sage: K = simplicial_sets.Simplex(5)
            sage: K.pushout()                                                           # needs sage.graphs
            Empty simplicial set

            sage: S0 = simplicial_sets.Sphere(0)
            sage: pt_map = S0.base_point_map()
            sage: pt_map.domain().pushout(pt_map) == S0                                 # needs sage.graphs
            True

            sage: K.pushout(K.constant_map(), pt_map)                                   # needs sage.graphs
            Traceback (most recent call last):
            ...
    ...
    def pullback(self, *maps):
        ...
        The universal property::
            ...
            ....:                S1.n_cells(1)[0]: K.n_cells(1)[0]})
            sage: D = S1.cone()          # the cone C(S^1)                              # needs sage.graphs
            sage: g = D.map_from_base()  # map from S^1 to C(S^1)                       # needs sage.graphs
            sage: P = T.product(D)                                                      # needs sage.graphs
            sage: h = P.universal_property(f, g)                                        # needs sage.graphs
            sage: h.domain() == S1                                                      # needs sage.graphs
            True
            sage: h.codomain() == P                                                     # needs sage.graphs
            True
    ...
    def wedge(self, *others):
        ...
        EXAMPLES::
            ...
            sage: W = X.wedge(Y)                                                        # needs sage.graphs
            sage: W.nondegenerate_simplices()                                           # needs sage.graphs
            [*, e, f]
            sage: W.homology()                                                          # needs sage.graphs
            {0: 0, 1: Z x Z}
            sage: S2 = simplicial_sets.Sphere(2)
            sage: X.wedge(S2).homology(reduced=False)                                   # needs sage.graphs
            {0: Z, 1: Z, 2: Z}
            sage: X.wedge(X).nondegenerate_simplices()                                  # needs sage.graphs
            [*, e, e]

            sage: S3 = simplicial_sets.Sphere(3)
            sage: W = S2.wedge(S3, S2)                                                  # needs sage.graphs
            sage: W.inclusion_map(2)                                                    # needs sage.graphs
            Simplicial set morphism:
              From: S^2
        ...
        which is isomorphic to ``S2``::
            ...
            sage: S2.f_vector()
            [1, 0, 1]
            sage: W.projection_map(2).codomain().f_vector()                             # needs sage.graphs
            [1, 0, 1]
            sage: (W.projection_map(2) * W.inclusion_map(2)).is_bijective()             # needs sage.graphs
            True

        TESTS::

            sage: Z = SimplicialSet({e: (v,w)})
            sage: X.wedge(Z)                                                            # needs sage.graphs
            Traceback (most recent call last):
            ...
    ...
    def cone(self):
        ...
        EXAMPLES::
            ...
            sage: e = AbstractSimplex(1, name='e')
            sage: X = SimplicialSet({e: (v, v)})
            sage: CX = X.cone()  # unreduced cone, since X not pointed                  # needs sage.graphs
            sage: CX.nondegenerate_simplices()                                          # needs sage.graphs
            [*, v, (v,*), e, (e,*)]
            sage: CX.base_point()                                                       # needs sage.graphs
            *

        `X` as a subset of the cone, and also the map from `X`, in the
        unreduced case::

            sage: CX.base_as_subset()                                                   # needs sage.graphs
            Simplicial set with 2 non-degenerate simplices
            sage: CX.map_from_base()                                                    # needs sage.graphs
            Simplicial set morphism:
        ...
        In the reduced case, only the map from `X` is available::
            ...
            sage: CX = X.cone()  # reduced cone                                         # needs sage.graphs
            sage: CX.nondegenerate_simplices()                                          # needs sage.graphs
            [*, e, (e,*)]
            sage: CX.map_from_base()                                                    # needs sage.graphs
            Simplicial set morphism:
    ...
    def suspension(self, n=1):
        ...
        EXAMPLES::
            ...
            sage: RP4 = simplicial_sets.RealProjectiveSpace(4)
            sage: S1 = simplicial_sets.Sphere(1)
            sage: SigmaRP4 = RP4.suspension()                                           # needs sage.graphs
            sage: S1_smash_RP4 = S1.smash_product(RP4)                                  # needs sage.graphs
            sage: SigmaRP4.homology() == S1_smash_RP4.homology()                        # needs sage.graphs
            True
        ...
        here::
            ...
            sage: SigmaRP4.f_vector()                                                   # needs sage.graphs sage.groups
            [1, 0, 1, 1, 1, 1]
            sage: S1_smash_RP4.f_vector()                                               # needs sage.graphs sage.groups
            [1, 1, 4, 6, 8, 5]

        TESTS::

            sage: RP4.suspension(-3)                                                    # needs sage.graphs sage.groups
            Traceback (most recent call last):
            ...
    ...
    def reduce(self):
        ...
        EXAMPLES::
            ...
            sage: K.is_reduced()
            False
            sage: X = K.reduce()                                                        # needs sage.graphs
            sage: X.is_reduced()                                                        # needs sage.graphs
            True

        ``X`` is reduced, so calling ``reduce`` on it again
        returns ``X`` itself::

            sage: X is X.reduce()                                                       # needs sage.graphs
            True
            sage: K is K.reduce()                                                       # needs sage.graphs
            False

        Raise an error for disconnected simplicial sets::

            sage: S0 = simplicial_sets.Sphere(0)
            sage: S0.reduce()                                                           # needs sage.graphs
            Traceback (most recent call last):
            ...
...
def shrink_simplicial_complex(K):
    ...
    EXAMPLES::
        ...
        sage: X = shrink_simplicial_complex(K)                                          # needs sage.graphs
        sage: X.f_vector()                                                              # needs sage.graphs
        [1]

        sage: Y = simplicial_complexes.Sphere(2)
        sage: S2 = shrink_simplicial_complex(Y); S2                                     # needs sage.graphs
        Quotient: (Simplicial set with
                   14 non-degenerate simplices/Simplicial set with
                                               13 non-degenerate simplices)
        sage: S2.f_vector()                                                             # needs sage.graphs
        [1, 0, 1]
        sage: S2.homology()                                                             # needs sage.graphs sage.modules
        {0: 0, 1: 0, 2: Z}
        ...
        {0: 0, 1: Z^6, 2: Z}
        sage: M = shrink_simplicial_complex(Z)                                          # needs sage.graphs
        sage: M.f_vector()  # random                                                    # needs sage.graphs
        [1, 32, 27]
        sage: M.homology()                                                              # needs sage.graphs sage.modules
        {0: 0, 1: Z^6, 2: Z}
    """
