...
full picture in 3D::
    ...
    sage: L = RootSystem(["A",2]).ambient_space()
    sage: L.plot(projection=False)                                                      # needs sage.plot sage.symbolic
    Graphics3d Object
...
`G_2`::
    ...
    sage: L.plot(reflection_hyperplanes="all")                                          # needs sage.plot sage.symbolic
...
its corresponding reflection hyperplane::
    ...
    sage: L.plot(roots="all", reflection_hyperplanes="all").show(figsize=15)            # needs sage.plot sage.symbolic
...
coordinates in the root lattice::
    ...
    sage: L = RootSystem(["G",2]).ambient_space()
    sage: L.plot(roots=list(Q.positive_roots()), fundamental_weights=False)             # needs sage.plot sage.symbolic
...
:wikipedia:`Wikipedia%27s E8 3D picture <File:E8_3D.png>`::
    ...
    sage: L.dimension()
    8
    sage: L.plot(roots="all", reflection_hyperplanes=False,             # long time, needs sage.plot sage.symbolic
...
sure everything fits in the picture::
    ...
    sage: RootSystem(["G",2]).ambient_space().plot(alcoves=True,                        # needs sage.plot sage.symbolic
...
The same picture in 3D, for type `B_3`::
    ...
    sage: RootSystem(["B",3]).ambient_space().plot(alcoves=True, alcove_labels=True)    # needs sage.plot sage.symbolic
    Graphics3d Object
...
We now draw the usual alcove picture for affine type `A_2^{(1)}`::
    ...
    sage: L.plot()                              # long time                             # needs sage.plot sage.symbolic
...
full picture in 3D::
    ...
    sage: L.plot(bounding_box=[[-3,3],[-3,3],[-1,1]], affine=False)     # long time, needs sage.plot sage.symbolic
    Graphics3d Object
...
reflection hyperplanes with the level `1` hyperplane::
    ...
    sage: L.plot(affine=False, level=1)                                 # long time, needs sage.plot sage.symbolic
    Graphics3d Object
...
level 1::
    ...
    sage: L.plot(affine=False, level=1)                                                 # needs sage.plot sage.symbolic
    Graphics3d Object
...
.. NOTE::
    ...
    method::
        ...
        sage: p = L.plot(affine=False, level=1)                                         # needs sage.plot sage.symbolic
        sage: p.show(aspect_ratio=[1,1,2], frame=False)                                 # needs sage.plot sage.symbolic
...
`u = vs_i`; the color of that wall is given by `i`::
    ...
    sage: L.plot(coroots="simple", alcove_labels=True)                  # long time, needs sage.plot sage.symbolic
...
small height in the root poset::
    ...
    sage: L.plot(roots=first_positive_roots, affine=False, alcoves=False)               # needs sage.plot sage.symbolic
...
Here is a polished solution for the first exercise::
    ...
    sage: p = L.plot(fundamental_chamber=True,                                          # needs sage.plot sage.symbolic
    ...
    sage: p.show(figsize=20)                                            # long time, needs sage.plot sage.symbolic
...
tiled by the alcoves, each of which is a 3D simplex::
    ...
    sage: L.plot(reflection_hyperplanes=False, bounding_box=85/100)     # long time, needs sage.plot sage.symbolic
    Graphics3d Object
...
the Weyl group::
    ...
    sage: W = L.weyl_group()
    sage: L.plot(reflection_hyperplanes=False, alcoves=[W.one()], bounding_box=2)       # needs sage.plot sage.symbolic
    Graphics3d Object
...
center in the root lattice::
    ...
    sage: W = L.weyl_group()
    sage: L.plot(reflection_hyperplanes=False, alcoves=[[0,0]], bounding_box=2)         # needs sage.plot sage.symbolic
    Graphics3d Object
...
which will be explained later on in this tutorial::
    ...
    sage: p = L.plot_fundamental_chamber("classical")                                   # needs sage.plot sage.symbolic
    sage: p += L.plot_alcoves(alcoves=alcoves, wireframe=True)                          # needs sage.plot sage.symbolic
    sage: p += L.plot_fundamental_weights()                                             # needs sage.plot sage.symbolic
    sage: p.show(frame=False)                                                           # needs sage.plot sage.symbolic
...
reflections, on top of the weight lattice in affine type `A_{2,1}`::
    ...
    sage: L.plot(alcove_walk=w1, bounding_box=6)        # long time                     # needs sage.plot sage.symbolic
...
below) that can be called independently and combined at will::
    ...
    sage: L.plot_roots() + L.plot_reflection_hyperplanes()                              # needs sage.plot sage.symbolic
...
.. NOTE::
    ...
    combining them. Here is a workaround::
        ...
        sage: p = L.plot_roots() + L.plot_reflection_hyperplanes()                      # needs sage.plot sage.symbolic
        sage: p.axes(False)                                                             # needs sage.plot sage.symbolic
        sage: p                                                                         # needs sage.plot sage.symbolic
        Graphics object consisting of 12 graphics primitives
...
and pass it down to each piece. We use this to plot our two walks::
    ...
    sage: p = L.plot_alcoves(plot_options=plot_options)                 # long time, needs sage.plot sage.symbolic
    sage: p += L.plot_alcove_walk(w1, color="green",                    # long time, needs sage.plot sage.symbolic
...
And another with some foldings::
    ...
    ....:                         color="purple")
    sage: p.axes(False)                                                                 # needs sage.plot sage.symbolic
    sage: p.show(figsize=20)                                                            # needs sage.plot sage.symbolic
...
the translation by this weight::
    ...
    sage: p = L.plot(plot_options=plot_options)                         # long time, needs sage.plot sage.symbolic
    sage: p += L.plot_alcove_walk(walk, color="green",                  # long time, needs sage.plot sage.symbolic
    ....:                         plot_options=plot_options)
    sage: p += plot_options.family_of_vectors({t: L(t)})                # long time, needs sage.plot sage.symbolic
    sage: plot_options.finalize(p)                                      # long time, needs sage.plot sage.symbolic
    Graphics object consisting of ... graphics primitives
    sage: p                                                             # long time, needs sage.plot sage.symbolic
    Graphics object consisting of ... graphics primitives
...
We conclude with a rank `3 + 1` alcove walk::
    ...
    sage: (L.plot_fundamental_weights()                                                 # needs sage.plot sage.symbolic
...
.. TOPIC:: Solution
    ...
    ::
        ...
        sage: L.plot_alcoves(alcoves=alcoves, color=color,              # long time, needs sage.plot sage.symbolic
...
Now you can create your own customized color Coxeter graph paper::
    ...
    sage: L = RootSystem(["C",2,1]).ambient_space()
    sage: p = L.plot(bounding_box=[[-8,9],[-5,7]],      # long time (10 s)              # needs sage.plot sage.symbolic
    ....:            coroots="simple")
    sage: p                                     # long time                             # needs sage.plot sage.symbolic
    Graphics object consisting of ... graphics primitives
...
postscript or svg before printing it::
    ...
    sage: p.save("C21paper.eps")                # not tested                            # needs sage.plot sage.symbolic
...
fundamental alcove::
    ...
    sage: p = L.plot_alcoves()                                                          # needs sage.plot sage.symbolic
    sage: p += g.plot(pos=positions, vertex_size=0,                                     # needs sage.plot sage.symbolic
...
The same picture for `A_3` gives a nice 3D permutohedron::
    ...
    sage: g = W.cayley_graph(side="right")
    sage: positions = {w: plot_options.projection(w.action(rho)) for w in W}
    sage: p = L.plot_roots()                                                            # needs sage.plot sage.symbolic
    sage: p += g.plot3d(pos3d=positions, color_by_label=plot_options.color)             # needs sage.plot sage.symbolic
    sage: p                                                                             # needs sage.plot sage.symbolic
    Graphics3d Object
...
according to its weight::
    ...
    sage: plot_options = L.plot_parse_options()                                         # needs sage.plot sage.symbolic

    sage: g = C.digraph()
    sage: positions = {x: plot_options.projection(x.weight()) for x in C}               # needs sage.plot sage.symbolic
    sage: p = L.plot()                                                                  # needs sage.plot sage.symbolic
    sage: p += g.plot(pos=positions,                                                    # needs sage.plot sage.symbolic
...
Here is an analogue picture in 3D::
    ...
    sage: g = C.digraph()
    sage: positions = {x: plot_options.projection(x.weight()) for x in C}               # needs sage.plot sage.symbolic
    sage: p = L.plot(reflection_hyperplanes=False, fundamental_weights=False)           # needs sage.plot sage.symbolic
    sage: p += g.plot3d(pos3d=positions, vertex_labels=True,                            # needs sage.plot sage.symbolic
...
def barycentric_projection_matrix(n, angle=0):
    ...
    Three vectors in dimension 2::
        ...
        sage: barycentric_projection_matrix(2)                                          # needs sage.symbolic
        [ 1/2*sqrt(3) -1/2*sqrt(3)            0]
        [         1/2          1/2           -1]

    Four vectors in dimension 3::

        sage: m = barycentric_projection_matrix(3); m                                   # needs sage.symbolic
    ...
    and have regular mutual angles::
        ...
        sage: m.transpose()*m                                                           # needs sage.symbolic
        [   1 -1/3 -1/3 -1/3]
        [-1/3    1 -1/3 -1/3]
        [-1/3 -1/3    1 -1/3]
        [-1/3 -1/3 -1/3    1]
    ...
    Here is a plot of them::
        ...
        sage: sum(arrow((0,0,0),x) for x in m.columns())                                # needs sage.plot sage.symbolic
        Graphics3d Object

    For 2D drawings of root systems, it is desirable to rotate the
    result to match with the usual conventions::

        sage: barycentric_projection_matrix(2, angle=2*pi/3)                            # needs sage.symbolic
        [         1/2           -1          1/2]
        [ 1/2*sqrt(3)            0 -1/2*sqrt(3)]

    TESTS::

        sage: for n in range(1, 7):                                                     # needs sage.symbolic
