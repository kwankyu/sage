    ...
    class ElementMethods:
        ...
        def factor(self, *args, **kwds):
            ...
            Here is an example to show that :trac:`7868` has been resolved::
                ...
                sage: f = x*y/(x+y)                                                     # needs sage.rings.finite_rings
        ...
        def partial_fraction_decomposition(self, decompose_powers=True):
            ...
            EXAMPLES::
                ...
                True
                sage: q = 1/(t^3+1) + 2/(t^2+2) + 3/(t-3)^5                             # needs sage.libs.pari
                sage: whole, parts = q.partial_fraction_decomposition(); parts          # needs sage.libs.pari
                [1/3/(t + 1), 3/(t^5 - 15*t^4 + 90*t^3 - 270*t^2 + 405*t - 243),
                 (-1/3*t + 2/3)/(t^2 - t + 1), 2/(t^2 + 2)]
                sage: sum(parts) == q                                                   # needs sage.libs.pari
                True
                sage: q = 2*t / (t + 3)^2                                               # needs sage.libs.pari
            ...
            And also over finite fields (see :trac:`6052`, :trac:`9945`)::
                ...
                sage: q = (x+1)/(x^3+x+1)                                               # needs sage.rings.finite_rings
                sage: q.partial_fraction_decomposition()                                # needs sage.rings.finite_rings
                (0, [(x + 1)/(x^3 + x + 1)])

                sage: R.<x> = GF(11)[]                                                  # needs sage.rings.finite_rings
                sage: q = x + 1 + 1/(x+1) + x^2/(x^3 + 2*x + 9)                         # needs sage.rings.finite_rings
                sage: q.partial_fraction_decomposition()                                # needs sage.rings.finite_rings
            ...
            Some special cases::
                ...
                sage: x.partial_fraction_decomposition()                                # needs sage.libs.pari
                (x, [])
                sage: R(0).partial_fraction_decomposition()                             # needs sage.libs.pari
                (0, [])
                sage: R(1).partial_fraction_decomposition()                             # needs sage.libs.pari
                (1, [])
