    ...
    class ElementMethods:
        def coproduct_iterated(self, n=1):
            ...
            EXAMPLES::
                ...
                sage: Psi[2,2].coproduct_iterated(0)                                    # needs sage.combinat
                Psi[2, 2]
                sage: Psi[2,2].coproduct_iterated(2)                                    # needs sage.combinat
            ...
            TESTS::
                ...
                sage: p[5,2,2].coproduct_iterated()                                     # needs sage.combinat
                p[] # p[5, 2, 2] + 2*p[2] # p[5, 2] + p[2, 2] # p[5]
                 + p[5] # p[2, 2] + 2*p[5, 2] # p[2] + p[5, 2, 2] # p[]
                sage: p([]).coproduct_iterated(3)                                       # needs sage.combinat
                p[] # p[] # p[] # p[]
            ...
            ::
                ...
                sage: Psi[2,2].coproduct_iterated(0)                                    # needs sage.combinat
                Psi[2, 2]
                sage: Psi[2,2].coproduct_iterated(3)                                    # needs sage.combinat
            ...
            ::
                ...
                sage: m[[1,3],[2]].coproduct_iterated(2)                                # needs sage.combinat
                m{} # m{} # m{{1, 3}, {2}} + m{} # m{{1}} # m{{1, 2}}
                 + m{} # m{{1, 2}} # m{{1}} + m{} # m{{1, 3}, {2}} # m{}
                 + m{{1}} # m{} # m{{1, 2}} + m{{1}} # m{{1, 2}} # m{}
                 + m{{1, 2}} # m{} # m{{1}} + m{{1, 2}} # m{{1}} # m{}
                 + m{{1, 3}, {2}} # m{} # m{}
                sage: m[[]].coproduct_iterated(3), m[[1,3],[2]].coproduct_iterated(0)   # needs sage.combinat
                (m{} # m{} # m{} # m{}, m{{1, 3}, {2}})
            """
            if n < 0:
