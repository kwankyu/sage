...
class GroupAlgebras(AlgebrasCategory):
    ...
    TESTS::
        ...
        sage: x = Permutation([4,3,2,1])                                                # needs sage.combinat
    ...
    class ParentMethods:
        ...
        def is_integral_domain(self, proof=True):
            ...
            EXAMPLES::
                ...
                sage: GroupAlgebra(S2).is_integral_domain()                             # needs sage.modules
                False
                sage: S1 = SymmetricGroup(1)
                sage: GroupAlgebra(S1).is_integral_domain()                             # needs sage.modules
                True
                sage: GroupAlgebra(S1, IntegerModRing(4)).is_integral_domain()          # needs sage.modules
                False
                sage: GroupAlgebra(AbelianGroup(1)).is_integral_domain()                # needs sage.modules
                True
                sage: GroupAlgebra(AbelianGroup(2, [0,2])).is_integral_domain()         # needs sage.modules
                False
                sage: GroupAlgebra(GL(2, ZZ)).is_integral_domain()      # not implemented, needs sage.modules
                False
            """
