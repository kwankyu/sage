...
class Category_over_base(CategoryWithParameters):
    ...
    EXAMPLES::
        ...
        sage: Algebras(GF(2)).element_class is Algebras(GF(3)).element_class            # needs sage.rings.finite_rings
        True

        sage: C = GF(2).category()                                                      # needs sage.rings.finite_rings
        sage: Algebras(GF(2)).parent_class is Algebras(C).parent_class                  # needs sage.rings.finite_rings
        True
    ...
    def _repr_object_names(self):
        ...
        EXAMPLES::
            ...
            sage: Algebras(GF(2).category())._repr_object_names()                       # needs sage.rings.finite_rings
...
class Category_over_base_ring(Category_over_base):
    def __init__(self, base, name=None):
        ...
        EXAMPLES::
            ...
            sage: C = Algebras(GF(2)); C                                                # needs sage.rings.finite_rings
            Category of algebras over Finite Field of size 2
            sage: TestSuite(C).run()                                                    # needs sage.rings.finite_rings
        """
    ...
    def base_ring(self):
        ...
        EXAMPLES::
            ...
            sage: C = Algebras(GF(2))                                                   # needs sage.rings.finite_rings
            sage: C.base_ring()                                                         # needs sage.rings.finite_rings
    ...
    def _subcategory_hook_(self, C):
        ...
        The answer is ``Unknown`` if ``C`` is not a category over base ring::
            ...
            Unknown
            sage: Sym = SymmetricFunctions(QQ)                                          # needs sage.combinat
            sage: from sage.combinat.sf.sfa import SymmetricFunctionsBases              # needs sage.combinat
            sage: Modules(QQ)._subcategory_hook_(SymmetricFunctionsBases(Sym))          # needs sage.combinat
            Unknown
            sage: SymmetricFunctionsBases(Sym).is_subcategory(Modules(QQ))              # needs sage.combinat
            True
        ...
        Case 3: the two bases are parents; then they should coincide::
            ...
            False
            sage: VectorSpaces(GF(2))._subcategory_hook_(Algebras(GF(3))) # base ring in the same category              # needs sage.rings.finite_rings
            False
        ...
        made respectively using the parent class or the base ring::
            ...
            False
            sage: issubclass(Algebras(GF(2)).parent_class,                              # needs sage.modules sage.rings.finite_rings
