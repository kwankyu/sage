...
- How to ensure consistency?
  ...
  Similar objects should behave similarly::
      ...
      sage: Permutations(5).cardinality()                                               # needs sage.combinat
      120
...
- How to reduce duplication?
  ...
  ::
      ...
      sage: m^8 == m*m*m*m*m*m*m*m == ((m^2)^2)^2                                       # needs sage.modules
      True
...
Those can be viewed graphically::
    ...
    sage: g = Groups().category_graph()                                                 # needs sage.graphs sage.modules
...
Applying an operation is generally done by *calling a method*::
    ...
    sage: pQ = R(p)                                                                     # needs sage.symbolic
    sage: type(pQ)                                                                      # needs sage.symbolic
    <class 'sage.rings.polynomial.polynomial_ring.PolynomialRing_field_with_category.element_class'>
    sage: pQ.factor()                                                                   # needs sage.symbolic
    (6) * (x + 1)^2

    sage: pZ = ZZ['x'](p)                                                               # needs sage.symbolic
...
the hierarchy of classes for an element of a group of permutations::
    ...
    sage: P = Permutations(4)                                                           # needs sage.combinat
    sage: m = P.an_element()                                                            # needs sage.combinat
    sage: for cls in m.__class__.mro(): print(cls)                                      # needs sage.combinat
...
group `G` is an instance of the following class::
    ...
    sage: G = GL(2, ZZ)                                                                 # needs sage.modules
    sage: type(G)                                                                       # needs sage.modules
...
Here is a piece of the hierarchy of classes above it::
    ...
    sage: for cls in G.__class__.mro(): print(cls)                                      # needs sage.modules
    <class 'sage.groups.matrix_gps.linear_gap.LinearMatrixGroup_gap_with_category'>
    ...
...
permutation groups (no surprise)::
    ...
    sage: P = PermutationGroup([[(1,2,3)]]); P                                          # needs sage.combinat
...
In this case, the group is commutative, so we can specify this::
    ...
    sage: P = PermutationGroup([[(1,2,3)]],                                             # needs sage.combinat
