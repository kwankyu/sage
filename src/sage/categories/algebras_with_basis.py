...
class AlgebrasWithBasis(CategoryWithAxiom_over_base_ring):
    ...
    computations with it::
        ...
        sage: A = C.example(); A                                                        # needs sage.combinat
        ...
        sage: A.one_basis()                                                             # needs sage.combinat
        word:
        sage: A.one()                                                                   # needs sage.combinat
        B[word: ]

        sage: A.base_ring()                                                             # needs sage.combinat
        Rational Field
        sage: A.basis().keys()                                                          # needs sage.combinat
        Finite words over {'a', 'b', 'c'}

        sage: (a,b,c) = A.algebra_generators()                                          # needs sage.combinat
        sage: a^3, b^2                                                                  # needs sage.combinat
        (B[word: aaa], B[word: bb])
        sage: a * c * b                                                                 # needs sage.combinat
        B[word: acb]

        sage: A.product                                                                 # needs sage.combinat
        <bound method MagmaticAlgebras.WithBasis.ParentMethods._product_from_product_on_basis_multiply of
         An example of an algebra with basis:
          the free algebra on the generators ('a', 'b', 'c') over Rational Field>
        sage: A.product(a * b, b)                                                       # needs sage.combinat
        B[word: abb]

        sage: TestSuite(A).run(verbose=True)                                            # needs sage.combinat
        ...
        sage: A.__class__                                                               # needs sage.combinat
        <class 'sage.categories.examples.algebras_with_basis.FreeAlgebra_with_category'>
        sage: A.element_class                                                           # needs sage.combinat
    ...
    def example(self, alphabet=('a','b','c')):
        ...
        EXAMPLES::
            ...
            sage: AlgebrasWithBasis(QQ).example()                                       # needs sage.combinat
            An example of an algebra with basis:
             the free algebra on the generators ('a', 'b', 'c') over Rational Field

        An other set of generators can be specified as optional argument::

            sage: AlgebrasWithBasis(QQ).example((1,2,3))                                # needs sage.combinat
            An example of an algebra with basis:
    ...
    class ParentMethods:
        ...
        def hochschild_complex(self, M):
            ...
            EXAMPLES::
                ...
                sage: R.<x> = QQ[]
                sage: A = algebras.DifferentialWeyl(R)                                  # needs sage.combinat
                sage: H = A.hochschild_complex(A)                                       # needs sage.combinat

                sage: SGA = SymmetricGroupAlgebra(QQ, 3)                                # needs sage.combinat
                sage: T = SGA.trivial_representation()                                  # needs sage.combinat
                sage: H = SGA.hochschild_complex(T)                                     # needs sage.combinat
            """
        ...
        class ParentMethods:
            ...
            def one_from_cartesian_product_of_one_basis(self):
                ...
                EXAMPLES::
                    ...
                    sage: A = AlgebrasWithBasis(QQ).example(); A                        # needs sage.combinat
                    An example of an algebra with basis: the free algebra
                     on the generators ('a', 'b', 'c') over Rational Field
                    sage: A.one_basis()                                                 # needs sage.combinat
                    word:

                    sage: B = cartesian_product((A, A, A))                              # needs sage.combinat
                    sage: B.one_from_cartesian_product_of_one_basis()                   # needs sage.combinat
                    B[(0, word: )] + B[(1, word: )] + B[(2, word: )]
                    sage: B.one()                                                       # needs sage.combinat
                    B[(0, word: )] + B[(1, word: )] + B[(2, word: )]

                    sage: cartesian_product([SymmetricGroupAlgebra(QQ, 3),              # needs sage.combinat
        ...
        class ParentMethods:
            ...
            def product_on_basis(self, t1, t2):
                ...
                EXAMPLES::
                    ...
                    sage: x = tensor((a, b, c)); x                                      # needs sage.combinat
                    B[word: a] # B[word: b] # B[word: c]
                    sage: y = tensor((c, b, a)); y                                      # needs sage.combinat
                    B[word: c] # B[word: b] # B[word: a]
                    sage: x * y                                                         # needs sage.combinat
                    B[word: ac] # B[word: bb] # B[word: ca]

                    sage: x = tensor(((a + 2*b), c)); x                                 # needs sage.combinat
                    B[word: a] # B[word: c] + 2*B[word: b] # B[word: c]
                    sage: y = tensor((c, a)) + 1; y                                     # needs sage.combinat
