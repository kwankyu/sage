...
overwritten::
    ...
    sage: import gc
    sage: _ = gc.collect()
    sage: from sage.rings.finite_rings.finite_field_prime_modn import FiniteField_prime_modn as FF  # needs sage.rings.finite_rings
    sage: L = [x for x in gc.get_objects() if isinstance(x, FF)]                        # needs sage.rings.finite_rings
    sage: len(L)                                                                        # needs sage.rings.finite_rings
    1
...
def Hom(X, Y, category=None, check=True):
    ...
    EXAMPLES::
        ...
        sage: Hom(ZZ, QQ, Sets())                                                       # needs sage.groups
    ...
    before :trac:`16275` (see :trac:`15801` for a real use case)::
        ...
        sage: S.category().is_subcategory(C)                                            # needs sage.graphs
        False
        sage: S in C                                                                    # needs sage.graphs
        True
    ...
    uninitialized parent::
        ...
        sage: P.<x,y> = QQ['x,y']
        sage: Q = P.quotient([x^2-1, y^2-1])                                            # needs sage.libs.singular
        ...
        si... = ... pg_Hom(si..., si..., ...) ...
        sage: Q == loads(dumps(Q))                                                      # needs sage.libs.singular
        True
...
def End(X, category=None):
    ...
    ::
        ...
        sage: G = AlternatingGroup(3)                                                   # needs sage.groups
        sage: S = End(G); S                                                             # needs sage.groups
        Set of Morphisms
         from Alternating group of order 3!/2 as a permutation group
         to Alternating group of order 3!/2 as a permutation group
         in Category of finite enumerated permutation groups
        sage: from sage.categories.homset import is_Endset
        sage: is_Endset(S)                                                              # needs sage.groups
        True
        sage: S.domain()                                                                # needs sage.groups
...
class Homset(Set_generic):
    ...
    Homsets of unique parents are unique as well::
        ...
        sage: H = End(AffineSpace(2, names='x,y'))                                      # needs sage.modules
        sage: loads(dumps(AffineSpace(2, names='x,y'))) is AffineSpace(2, names='x,y')  # needs sage.modules
        True
        sage: loads(dumps(H)) is H                                                      # needs sage.modules
        True
    ...
    def _abstract_element_class(self):
        ...
        this point this is the simplest one to create (gosh)::
            ...
            sage: C3 = PermutationGroup([(1,2,3)])                                      # needs sage.groups
            sage: C3._refine_category_(cat)                                             # needs sage.groups
            sage: C2 = PermutationGroup([(1,2)])                                        # needs sage.groups
            sage: C2._refine_category_(cat)                                             # needs sage.groups
            sage: H = Hom(C3, C2, cat)                                                  # needs sage.groups
            sage: H.homset_category()                                                   # needs sage.groups
            Category of finite commutative groups
            sage: H.category()                                                          # needs sage.groups
            Category of homsets of unital magmas
            sage: cls = H._abstract_element_class; cls                                  # needs sage.groups
            <class 'sage.categories.homsets.GroupHomset_libgap_with_category._abstract_element_class'>
            sage: cls.__bases__ == (H.category().element_class, H.homset_category().morphism_class)                     # needs sage.groups
            True
    ...
    def natural_map(self):
        ...
        EXAMPLES::
            ...
            Coercion morphism:
              From: Univariate Polynomial Ring in t over Integer Ring
              To:   Univariate Polynomial Ring in t over Rational Field
            sage: H = Hom(QQ['t'], GF(3)['t'])                                          # needs sage.rings.finite_rings
            sage: H.natural_map()                                                       # needs sage.rings.finite_rings
            Traceback (most recent call last):
            ...
