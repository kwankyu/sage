    ...
    class ParentMethods:
        ...
        def from_vector(self, vector, order=None, coerce=True):
            ...
            EXAMPLES::
                ...
                sage: A = algebras.FiniteDimensional(                                   # needs sage.combinat
                ....:         QQ, [p_mult, q_mult, matrix(QQ, 3, 3)], 'p,q,z')
                sage: A.from_vector(vector([1,0,2]))                                    # needs sage.combinat
                p + 2*z
            """
            if order is None:
                try:
        ...
        def echelon_form(self, elements, row_reduced=False, order=None):
            ...
            ::
                ...
                sage: C.echelon_form([x[0] - x[1], 2*x[1] - 2*x[2], x[0] - x[2]])       # needs sage.modules
                [x[0] - x[2], x[1] - x[2]]
            ...
            We convert the input elements to ``self``::
                ...
                sage: E.echelon_form([1, x + 2])                                        # needs sage.combinat sage.modules
                [1, x]
            """
        ...
        def invariant_module(self, S, action=operator.mul, action_on_basis=None,
            ...
            of the symmetric group::
                ...
                sage: G = SymmetricGroup(3); G.rename('S3')                             # needs sage.groups sage.modules
                sage: M = FreeModule(ZZ, [1,2,3], prefix='M'); M.rename('M')            # needs sage.groups sage.modules
                sage: action = lambda g, x: M.term(g(x))
                sage: I = M.invariant_module(G, action_on_basis=action); I              # needs sage.groups sage.modules
                (S3)-invariant submodule of M
                sage: I.basis()                                                         # needs sage.groups sage.modules
                Finite family {0: B[0]}
                sage: [I.lift(b) for b in I.basis()]                                    # needs sage.groups sage.modules
                [M[1] + M[2] + M[3]]

                sage: G.rename(); M.rename()  # reset the names                         # needs sage.groups sage.modules
            ...
            construct the `H`-invariant module of the group algebra `\QQ[G]`::
                ...
                sage: I = A.invariant_module(H)                                         # needs sage.modules
                sage: [I.lift(b) for b in I.basis()]                                    # needs sage.modules
                [() + (1,2,3,4) + (1,3)(2,4) + (1,4,3,2),
                 (2,4) + (1,2)(3,4) + (1,3) + (1,4)(2,3)]
                sage: all(h * I.lift(b) == I.lift(b)                                    # needs sage.modules
        ...
        def twisted_invariant_module(self, G, chi,
            ...
            EXAMPLES::
                ...
                sage: M = CombinatorialFreeModule(QQ, [1,2,3])                          # needs sage.groups sage.modules
    ...
    class MorphismMethods:
        ...
        def __invert__(self):
            ...
            EXAMPLES::
                ...
                sage: X = CombinatorialFreeModule(ZZ, [1,2], category=category); X.rename("X"); x = X.basis()           # needs sage.modules
                sage: Y = CombinatorialFreeModule(ZZ, [3,4], category=category); Y.rename("Y"); y = Y.basis()           # needs sage.modules
                sage: phi = X.module_morphism(on_basis={1: y[3] + 3*y[4], 2: 2*y[3] + 5*y[4]}.__getitem__,              # needs sage.modules
                ....:                         codomain=Y, category=category)
                sage: psi = ~phi                                                                                        # needs sage.modules
                sage: psi                                                                                               # needs sage.modules
                Generic morphism:
                  From: Y
                  To:   X
                sage: psi.parent()                                                                                      # needs sage.modules
                Set of Morphisms from Y to X in Category of finite dimensional modules with basis over Integer Ring
                sage: psi(y[3])                                                                                         # needs sage.modules
                -5*B[1] + 3*B[2]
                sage: psi(y[4])                                                                                         # needs sage.modules
                2*B[1] - B[2]
                sage: psi.matrix()                                                                                      # needs sage.modules
                [-5  2]
                [ 3 -1]
                sage: psi(phi(x[1])), psi(phi(x[2]))                                                                    # needs sage.modules
                (B[1], B[2])
                sage: phi(psi(y[3])), phi(psi(y[4]))                                                                    # needs sage.modules
                (B[3], B[4])
