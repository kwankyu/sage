...
class ComplexReflectionGroups(Category_singleton):
    ...
    An example of a reflection group::
        ...
        sage: W = ComplexReflectionGroups().example(); W                                # needs sage.combinat sage.groups
        5-colored permutations of size 3

    ``W`` is in the category of complex reflection groups::

        sage: W in ComplexReflectionGroups()                                            # needs sage.combinat sage.groups
        True

    TESTS::

        sage: TestSuite(W).run()                                                        # needs sage.combinat sage.groups
        sage: TestSuite(ComplexReflectionGroups()).run()                                # needs sage.combinat sage.groups
    """
    ...
    def example(self):
        ...
        EXAMPLES::
            ...
            sage: from sage.categories.complex_reflection_groups import ComplexReflectionGroups
            sage: ComplexReflectionGroups().example()                                   # needs sage.combinat sage.groups
            5-colored permutations of size 3
        """
    ...
    class ParentMethods:
        ...
        def rank(self):
            ...
            EXAMPLES::
                ...
                sage: W = CoxeterGroups().example(); W                                  # needs sage.groups
                The symmetric group on {0, ..., 3}
                sage: W.rank()                                                          # needs sage.groups
                3
            """
