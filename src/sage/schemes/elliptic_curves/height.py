...
class EllipticCurveCanonicalHeight:
    ...
    def real_intersection_is_empty(self, Bk, v):
        ...
        for 0.08::
            ...
            sage: H.real_intersection_is_empty([H.B(n,0.07) for n in srange(1,5)], v)  # long time (3.3s), needs sage.rings.number_field
            True
    ...
    def complex_intersection_is_empty(self, Bk, v, verbose=False, use_half=True):
        ...
        sufficient::
            ...
            sage: H.complex_intersection_is_empty([H.B(n,0.02) for n in [1,2,3]], v)  # long time (~6s), needs sage.rings.number_field
            False
            sage: H.complex_intersection_is_empty([H.B(n,0.02) for n in [1,2,3,4]], v)  # needs sage.rings.number_field
            True
            sage: H.complex_intersection_is_empty([H.B(n,0.03) for n in [1,2,3,4]], v)  # long time (4s), needs sage.rings.number_field
            False
    ...
    def min(self, tol, n_max, verbose=False):
        ...
        After base change the lower bound can decrease::
            ...
            sage: E.change_ring(K).height_function().min(0.5, 10)       # long time (8s), needs sage.rings.number_field
            0.04419417382415922
