...
class EllipticCurvePoint_field(SchemeMorphism_point_abelian_variety_field):
    ...
    ::
        ...
        sage: P.order() == Q.order() == 4       # long time (3s)                        # needs sage.rings.number_field
        True
...
class EllipticCurvePoint_number_field(EllipticCurvePoint_field):
    ...
    def elliptic_logarithm(self, embedding=None, precision=100,
        ...
        Examples over number fields::
            ...
            sage: E = EllipticCurve([-a^2 - a - 1, a^2 + a])                            # needs sage.rings.number_field
            sage: Ls = [E.period_lattice(e) for e in embs]                              # needs sage.rings.number_field
    ...
    def padic_elliptic_logarithm(self, p, absprec=20):
        ...
        EXAMPLES::
            ...
            sage: [(2*P).padic_elliptic_logarithm(p)/P.padic_elliptic_logarithm(p) for p in prime_range(20)]  # long time (3s), needs sage.rings.padics
            [2 + O(2^19), 2 + O(3^20), 2 + O(5^19), 2 + O(7^19), 2 + O(11^19), 2 + O(13^19), 2 + O(17^19), 2 + O(19^19)]
            sage: [(3*P).padic_elliptic_logarithm(p)/P.padic_elliptic_logarithm(p) for p in prime_range(12)]  # long time (2s), needs sage.rings.padics
            [1 + 2 + O(2^19), 3 + 3^20 + O(3^21), 3 + O(5^19), 3 + O(7^19), 3 + O(11^19)]
            sage: [(5*P).padic_elliptic_logarithm(p)/P.padic_elliptic_logarithm(p) for p in prime_range(12)]  # long time (2s), needs sage.rings.padics
